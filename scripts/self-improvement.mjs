...
function suggestion(key, occurrences, reports) {
  ...
  if (key.includes("npm ci") || key.includes("lock file") || key.includes("desincroniz")) return { ...common, title: "Detectar sincronización package-lock antes de CI", rationale: "La instalación limpia falla antes de las validaciones cuando el lockfile queda desactualizado.", likely_files: ["package.json", "package-lock.json", ".github/workflows"], proposed_tests: ["npm ci sobre checkout limpio", "comparar package.json y lockfile en CI"] };
  if (key.includes("eslint") || key.includes("lint")) return { ...common, title: "Consolidar configuración lint", rationale: "Los fallos de lint deben señalar configuración o código sin desactivar reglas.", likely_files: ["eslint.config.js", "scripts"], proposed_tests: ["npm run lint", "verificar que no existan supresiones globales"] };
  return { ...common, title: `Investigar patrón recurrente: ${key.slice(0, 100)}`, rationale: "El mismo síntoma aparece en reportes históricos y merece una refactorización pequeña y demostrable.", likely_files: [], proposed_tests: ["reproducir el fallo", "validar lint, tests y build"] };
}
