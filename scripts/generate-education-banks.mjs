  ...
  const prompt = {
    ...
    messages: [
      ...
      { role: "user", content: JSON.stringify({
        ...
        schema: { questions: [{ text: "pregunta en español", options: ["opción A", "opción B", "opción C", "opción D"], answer: "una de las cuatro opciones exactamente", levels: ["primaria"], difficulty: "facil" }] },
        forbidden: existing
      }) }
    ]
  };
  const response = await fetch(MODEL_URL, { method: "POST", headers: { Authorization: `Bearer ${process.env.GITHUB_TOKEN}`, "Content-Type": "application/json", Accept: "application/vnd.github+json" }, body: JSON.stringify(prompt) });
...
function escapeTs(s) { return JSON.stringify(s); }
function render(bankName, topics, all) {
  const lines = ['import type { EducationDifficulty, EducationLevel } from "./education-engine";', '', 'type Q={text:string;options:string[];answer:string;levels:EducationLevel[];difficulty:EducationDifficulty};', 'const q=(text:string,options:string[],answer:string,levels:EducationLevel[],difficulty:EducationDifficulty):Q=>({text,options,answer,levels,difficulty});', '', `export const EDUCATION_BANK_${bankName}:Record<string,Q[]>={`];
  for (const topic of topics) {
    lines.push(` ${JSON.stringify(topic)}:[`);
    for (const item of all[topic]) lines.push(`  q(${escapeTs(item.text)},[${item.options.map(escapeTs).join(",")}],${escapeTs(item.answer)},[${escapeTs(item.levels[0])}],${escapeTs(item.difficulty)}),`);
    lines.push(" ],");
  }
  lines.push("};", "");
  return lines.join("\n");
  ...
  for (const topic of topics) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        console.warn(`${file}: ${topic}: intento ${attempt} falló; reintentando...`);
      }
    }
...
// Atomic write: never replace a real bank until every topic in both banks passed validation.
const tempA = `${ROOT}/.education-bank-a.generated.ts`;
const tempB = `${ROOT}/.education-bank-b.generated.ts`;
try {
  await fs.writeFile(tempA, render("A", Object.keys(generated["education-bank-a.ts"]), generated["education-bank-a.ts"]), "utf8");
  await fs.writeFile(tempB, render("B", Object.keys(generated["education-bank-b.ts"]), generated["education-bank-b.ts"]), "utf8");
  await fs.rename(tempA, `${ROOT}/education-bank-a.ts`);
  await fs.rename(tempB, `${ROOT}/education-bank-b.ts`);
  console.log("Todos los bancos fueron generados con 20 preguntas independientes por tema.");
} catch (error) {
  await Promise.all([fs.rm(tempA, { force: true }), fs.rm(tempB, { force: true })]);
  throw error;
}
