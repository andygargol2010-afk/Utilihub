...
function SpanishToolPage() {
  ...
  return <main className="container-page py-6 sm:py-8">
    ...
    <section data-tool-surface aria-label={`Herramienta: ${spanishToolName(tool)}`} className="surface-card mt-5 p-4 sm:p-5">{ui ? ui() : <p role="alert" className="text-muted-foreground">Herramienta no disponible.</p>}<div className="mt-4"><ShareAndExportActions title={spanishToolName(tool)} /></div></section>
    <AdsterraBanner />
  </main>;
}
