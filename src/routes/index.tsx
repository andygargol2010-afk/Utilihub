...
const description = cleanDescription("500+ free online tools to calculate, convert, study, and handle everyday tasks. Find the tool you need with no signup.");
...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [{ title }, { name: "description", content: description }, { name: "keywords", content: "online tools, online calculators, converters, free tools, UtiliHub" }, { name: "robots", content: "index, follow, max-image-preview:large" }, { property: "og:title", content: title }, { property: "og:description", content: description }, { property: "og:type", content: "website" }, { property: "og:url", content: absoluteUrl("/") }, { property: "og:site_name", content: SITE_NAME }, { property: "og:image", content: ogImage() }, { name: "twitter:card", content: "summary_large_image" }, { name: "twitter:title", content: title }, { name: "twitter:description", content: description }, { name: "twitter:image", content: ogImage() }], links: [{ rel: "canonical", href: absoluteUrl("/") }, { rel: "alternate", hrefLang: "en", href: absoluteUrl("/") }, { rel: "alternate", hrefLang: "es", href: absoluteUrl("/es") }, { rel: "alternate", hrefLang: "x-default", href: absoluteUrl("/") }], scripts: [{ type: "application/ld+json", children: JSON.stringify(websiteSchema()) }] }),
  component: HomePage,
});

function HomePage() {
  const quickTools = QUICK_TOOL_SLUGS.map((s) => ALL_TOOLS.find((t) => t.slug === s)).filter(Boolean) as typeof ALL_TOOLS;
      ...
      <section className="hero-gradient border-b border-border/70">
        <div className="container-page grid gap-8 py-10 sm:py-16 lg:grid-cols-[1.1fr_.9fr] lg:items-center lg:gap-12 lg:py-20">
          <div>
            ...
            <h1 className="max-w-3xl text-4xl font-black tracking-tight sm:text-5xl lg:text-6xl">Solve what you need in seconds.</h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg">
              Calculate, convert, study, and organize everyday tasks with free tools that run right in your browser.
            </p>
            <div className="mt-7 flex flex-wrap gap-3">
              <Link to="/tools" className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground shadow-lift hover:-translate-y-0.5 hover:opacity-95">
                Browse tools <ArrowRight className="size-4" />
              </Link>
              <Link to="/finance" className="inline-flex min-h-12 items-center gap-2 rounded-xl border border-border bg-card px-5 py-3 text-sm font-bold hover:border-primary/40 hover:bg-accent">
                View financial calculators
              </Link>
            </div>
            <div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 text-xs font-semibold text-muted-foreground">
              <span className="flex items-center gap-1.5"><Check className="size-3.5 text-primary" /> No signup</span>
              <span className="flex items-center gap-1.5"><Check className="size-3.5 text-primary" /> Free</span>
              <span className="flex items-center gap-1.5"><Check className="size-3.5 text-primary" /> Local privacy</span>
            </div>
          </div>
          <div className="surface-card bg-card/80 p-3 shadow-lift sm:p-4">
            <div className="rounded-xl border border-border/70 bg-background p-4 sm:p-5">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 text-xs font-semibold sm:grid-cols-3">
                ...
                <Link to="/category/text" className="rounded-lg border border-border/70 bg-card px-3 py-2.5 hover:border-primary/40 hover:bg-accent">Text</Link>
                <Link to="/category/developer" className="rounded-lg border border-border/70 bg-card px-3 py-2.5 hover:border-primary/40 hover:bg-accent">Developer</Link>
                <Link to="/finance" className="rounded-lg border border-border/70 bg-card px-3 py-2.5 hover:border-primary/40 hover:bg-accent">Finance</Link>
                <Link to="/category/converters" className="rounded-lg border border-border/70 bg-card px-3 py-2.5 hover:border-primary/40 hover:bg-accent">Converters</Link>
                <Link to="/games" className="rounded-lg border border-border/70 bg-card px-3 py-2.5 hover:border-primary/40 hover:bg-accent">Games</Link>
              </div>
            </div>
          </div>
        </div>
        ...
        <section className="pt-14 pb-10 sm:pt-16 sm:pb-12" aria-labelledby="shortcuts-title">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 id="shortcuts-title" className="mt-1 text-2xl font-black sm:text-3xl">Quick links</h2>
              <p className="mt-2 text-sm text-muted-foreground">The most practical tools to start without browsing the full catalog.</p>
            </div>
            <Link to="/tools" className="hidden min-h-11 items-center gap-1 rounded-lg px-3 text-sm font-bold text-primary hover:bg-accent sm:inline-flex">
        ...
        <section className="mt-10 border-t border-border/70 py-10" aria-labelledby="explore-categories">
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <h2 id="explore-categories" className="mt-1 text-2xl font-black">All categories</h2>
            ...
            {categories.map((category) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="ml-3 shrink-0 rounded-full bg-surface px-2.5 py-1 text-xs font-bold text-muted-foreground">{count}</span>
                </a>
              );
            })}
          </div>
