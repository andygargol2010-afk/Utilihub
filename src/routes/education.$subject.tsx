...
export const Route = createFileRoute("/education/$subject")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => {
    ...
    return { meta: [
      ...
      breadcrumbSchema([{ name: "Home", path: "/" }, { name: "Education", path: "/category/educacion" }, { name: name }]),
    ] }) }] };
  },
...
function EducationSubjectPage() {
  ...
  return <div className="container-page py-10">
    <Breadcrumbs items={[{ label: "Home", to: "/" }, { label: "Education", to: "/category/$slug", params: { slug: "educacion" } }, { label: name }]} />
