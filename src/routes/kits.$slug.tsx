...
export const Route = createFileRoute("/kits/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) return { meta: [{ title: "Kit not found | UtiliHub" }, { name: "robots", content: "noindex, nofollow" }] };
...
function KitPage() {
  ...
  return <main className="container-page py-6 sm:py-8">
    ...
    <section className="mt-8" aria-labelledby="kit-steps"><div className="flex items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-primary">Recommended path</p><h2 id="kit-steps" className="mt-1 text-2xl font-black">Follow these steps</h2></div><Link to="/herramientas" className="hidden items-center gap-1 text-sm font-bold text-primary sm:inline-flex">View full catalog <ArrowRight className="size-4" /></Link></div><div className="mt-5 space-y-3">{tools.map((tool, index) => <div key={tool.slug} className="surface-card flex gap-4 p-4 sm:p-5"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-primary text-sm font-black text-primary-foreground">{index + 1}</span><div className="min-w-0 flex-1"><ToolCard tool={tool} /><Link to={toolHref(tool) as "/herramientas/$slug"} params={{ slug: tool.slug }} className="mt-3 inline-flex items-center gap-1 text-sm font-bold text-primary">Open tool <ArrowRight className="size-4" /></Link></div></div>)}</div></section>
  </main>;
}
