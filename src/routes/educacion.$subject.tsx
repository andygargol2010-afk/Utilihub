...
export const Route = createFileRoute("/educacion/$subject")({
  ...
  head: ({ loaderData }) => {
    ...
    return { meta: [
      { title }, { name: "description", content: description }, { name: "keywords", content: `tests de ${name}, ejercicios de ${name}, preguntas de ${name}, examen de ${name}, educación online` }, { name: "robots", content: "index, follow, max-image-preview:large" },
      ...
      { "@type": "CollectionPage", name: title, description, url, isPartOf: { "@type": "WebSite", name: SITE_NAME, url: absoluteUrl("/") } },
      breadcrumbSchema([{ name: "Inicio", path: "/" }, { name: "Educación", path: "/categoria/educacion" }, { name }]),
    ] }) }] };
  },
...
function EducationSubjectPage() {
  ...
  return <div className="container-page py-10"><Breadcrumbs items={[{ label: "Inicio", to: "/" }, { label: "Educación", to: "/categoria/$slug", params: { slug: "educacion" } }, { label: name }]} /><h1 className="mt-4 text-3xl font-bold sm:text-4xl">Tests de {name}</h1><p className="mt-3 max-w-3xl text-muted-foreground">Genera evaluaciones gratuitas de {name} por tema. Puedes elegir primaria, secundaria o universidad, además de dificultad y cantidad de preguntas.</p><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{tools.map((tool) => <ToolCard key={tool.slug} tool={tool} />)}</div><p className="mt-10 text-sm text-muted-foreground">Temas disponibles: {tools.map((tool) => tool.name.replace(/^Creador de tests: /, "")).map((topic) => educationTopicTitle(topic)).join(", ")}.</p></div>;
}
