...
function RootComponent() { const { queryClient } = Route.useRouteContext(); const { pathname } = useLocation(); useShareableParams(); const { recordActivity } = useDailyStreak(); useEffect(() => { recordActivity(); }, [pathname, recordActivity]); return <QueryClientProvider client={queryClient}><a href="#contenido-principal" className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[100] focus:rounded-lg focus:bg-primary focus:px-4 focus:py-3 focus:text-sm focus:font-bold focus:text-primary-foreground">Saltar al contenido</a><SiteHeader /><div id="container-e558e6b6ef882881933f1f601fb945c1" className="mx-auto min-h-[1px] w-full max-w-6xl px-4" /><main id="contenido-principal"><Outlet /></main><SiteFooter /><Analytics /><SpeedInsights /></QueryClientProvider>; }
