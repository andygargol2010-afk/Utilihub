...
function SpanishFinanceTool() {
  ...
  return <main className="container-page py-10 sm:py-14"><div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[.14em] text-primary">Calculadora financiera</p><h1 className="mt-1 text-3xl font-black tracking-tight sm:text-4xl">{name}</h1><p className="mt-3 text-base leading-7 text-muted-foreground">Calcula y analiza este escenario financiero directamente en tu navegador.</p></div><FavoriteButton slug={tool.slug} name={name} /><section data-tool-surface className="surface-card mt-8 p-5 sm:p-7" aria-label={name}>{ui()}<ShareAndExportActions title={name} locale="es" /></section><AdsterraBanner /><section className="mt-8 grid gap-8 border-t border-border/70 pt-8 lg:grid-cols-2"><div><h2 className="text-xl font-black">Sobre esta calculadora</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Esta herramienta realiza un cálculo matemático a partir de los valores que introduces y funciona directamente en el navegador.</p></div><div><h2 className="text-xl font-black">Cómo usarla</h2><ol className="mt-3 space-y-3 text-sm leading-6 text-muted-foreground"><li>1. Introduce los valores del escenario que quieres analizar.</li><li>2. Ejecuta el cálculo y revisa los resultados.</li><li>3. Comprueba las condiciones reales antes de tomar decisiones.</li></ol></div></section></main>;
}
