...
function SpanishHome() {
  ...
  return <main className="pb-12">
    ...
    <div className="container-page">
      <section className="py-10 sm:py-12" aria-labelledby="es-categories"><p className="text-xs font-bold uppercase tracking-[.14em] text-primary">Explora</p><h2 id="es-categories" className="mt-1 text-2xl font-black">Categorías</h2><div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{categories.map((category) => <a key={category.slug} href={category.slug === "finanzas" ? "/es/finanzas" : "/es/herramientas"} className="group flex min-h-16 items-center justify-between rounded-xl border border-border/70 bg-card px-4 py-3 hover:border-primary/40 hover:bg-accent"><span className="font-bold group-hover:text-primary">{spanishCategoryName(category.slug)}</span><span className="text-xs text-muted-foreground">Ver</span></a>)}</div></section>
