...
export const Route = createFileRoute("/herramientas/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const url = absoluteUrl(`/herramientas/${tool.slug}`);
    return {
      meta: [
        { title: tool.title },
      ...
      scripts: [{ type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@graph": [
        ...
        breadcrumbSchema([{ name: "Inicio", path: "/" }, { name: "Herramientas", path: "/herramientas" }, { name: category.name, path: `/categoria/${category.slug}` }, { name: tool.name }]),
      ] }) }],
    };
  },
  component: ToolPage,
...
function ToolPage() {
  ...
  return <div className="container-page py-10">
    ...
    <section aria-label={`Herramienta: ${tool.name}`} className="surface-card mt-8 p-5 sm:p-7">{ui ? ui() : <p className="text-muted-foreground">Herramienta no disponible.</p>}</section>
    <div className="mt-12 grid gap-10 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]"><div className="space-y-10">
      ...
      {tool.faq?.length ? <section><h2 className="text-2xl font-semibold">Preguntas frecuentes</h2><dl className="mt-4 divide-y divide-border">{tool.faq.map((f) => <div key={f.q} className="py-4"><dt className="font-medium">{f.q}</dt><dd className="mt-1 text-muted-foreground">{f.a}</dd></div>)}</dl></section> : null}
    </div><aside className="space-y-4"><h2 className="text-lg font-semibold">Herramientas relacionadas</h2>{related.map((t) => <ToolCard key={t.slug} tool={t} />)}<a href={`/categoria/${category.slug}`} className="inline-block text-sm font-medium text-primary hover:underline">Ver toda la categoría {category.name} →</a></aside></div>
  </div>;
}
