...
export const Route = createFileRoute("/herramientas/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!category) {
      return {
        meta: [{ title: tool.title }, { name: "description", content: tool.description }],
      };
    }
    ...
    <div className="container-page py-10">
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-muted-foreground">{tool.summary}</p>
        </div>
      ...
      <section
        ...
        className="surface-card mt-8 p-5 sm:p-7"
      >
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        <div className="space-y-10">
          ...
          <section>
            <h2 className="text-2xl font-semibold">Cómo usar {tool.name.toLowerCase()}</h2>
        ...
        <aside className="space-y-4">
          <h2 className="text-lg font-semibold">Herramientas relacionadas</h2>
          {related.map((t) => (
