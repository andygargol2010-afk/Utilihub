...
import { ALL_CATEGORIES, allToolBySlug, allToolsByCategory, toolHref } from "@/lib/all-tools";
...
function ToolPage() {
  ...
  return <main className="container-page py-6 sm:py-8">
    ...
    {nextStep && <section className="mt-5 rounded-2xl border border-primary/20 bg-accent/50 p-4 sm:p-5" aria-labelledby="siguiente-paso"><p className="text-xs font-bold uppercase tracking-[.14em] text-primary">Siguiente paso recomendado</p><div className="mt-2 flex flex-wrap items-center justify-between gap-3"><div><h2 id="siguiente-paso" className="text-base font-black">Continúa con {nextStep.name}</h2><p className="mt-1 text-sm text-muted-foreground">Está relacionada con esta herramienta y puede ayudarte a completar la tarea.</p></div><Link to={toolHref(nextStep) as "/herramientas/$slug"} params={{ slug: nextStep.slug }} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground hover:opacity-90">Abrir siguiente <ArrowRight className="size-4" /></Link></div></section>}
