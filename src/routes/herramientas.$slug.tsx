...
function ToolPage() {
  ...
  return <div className="container-page py-10"><Breadcrumbs items={[{ label: "Inicio", to: "/" }, { label: "Herramientas", to: "/herramientas" }, { label: category.name, to: "/categoria/$slug", params: { slug: category.slug } }, { label: tool.name }]} />
    ...
    <section data-tool-surface aria-label={`Herramienta: ${tool.name}`} className="surface-card mt-8 p-5 sm:p-7">{ui ? ui() : <p className="text-muted-foreground">Herramienta no disponible.</p>}<ShareAndExportActions title={tool.name} /></section>
