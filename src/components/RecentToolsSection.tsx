  ...
  return (
    <section className="container-page pb-14" aria-labelledby="recent">
      <div className="flex items-end justify-between gap-4">
        <div>
          <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-[.16em] text-primary">
            <Clock3 className="size-3.5" /> {isSpanish ? "Tu actividad" : "Your activity"}
          </p>
          <h2 id="recent" className="mt-2 text-2xl font-extrabold sm:text-3xl">
            {isSpanish ? "Usadas recientemente" : "Recently used"}
          </h2>
          <p className="mt-2 text-sm text-muted-foreground">
            {isSpanish ? "Vuelve rápidamente a las herramientas que acabas de usar." : "Quickly return to the tools you just used."}
          </p>
        </div>
      </div>
        ...
        {tools.map((tool) => {
          ...
          return (
            <Link
              ...
              to={to}
              params={{ slug }}
              className="surface-card group p-4 transition hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lift"
            >
