...
export function SudokuGame({ locale = "en" }: { locale?: GameLocale }) {
  ...
  const newGame = useCallback((d: Diff) => {
    const g = generate(d);
    setDiff(d); setGame(g); setGrid(g.puzzle.map((r) => r.slice()));
    setFixed(g.puzzle.map((row) => row.map((n) => n !== 0)));
    setSelected(null); setSeconds(0); setRunning(true); setWon(false); setMsg("");
    setBest(readBestLow(`sudoku_${d}`)); setTick((t) => t + 1);
  }, []);
  ...
  const check = () => {
    ...
    setMsg(errors === 0 ? (es ? "Sin errores (puede faltar completar)." : "No mistakes so far.") : (es ? `${errors} casilla(s) incorrecta(s)` : `${errors} incorrect cell(s)`));
  };
  ...
  const hint = () => {
    ...
    setGrid((g) => { const next = g.map((row) => row.slice()); next[r]![c] = solution[r]![c]!; return next; });
    setSelected([r, c]);
  };
  ...
  return (
    <div className="mx-auto flex max-w-sm flex-col items-center gap-3">
      <div className="flex flex-wrap justify-center gap-2 text-sm font-semibold text-emerald-100/90">
        <GameSecondaryButton active={diff==="easy"} onClick={()=>newGame("easy")}>{es?"Fácil":"Easy"}</GameSecondaryButton>
        <GameSecondaryButton active={diff==="medium"} onClick={()=>newGame("medium")}>{es?"Medio":"Med"}</GameSecondaryButton>
        <GameSecondaryButton active={diff==="hard"} onClick={()=>newGame("hard")}>{es?"Difícil":"Hard"}</GameSecondaryButton>
      ...
      <div className="grid grid-cols-9 gap-0.5 rounded-xl bg-emerald-950/50 p-1.5" key={tick}>
        {grid.map((row, r) => row.map((v, c) => {
          ...
          const border = (c%3===2&&c!==8?"border-r-2 border-r-emerald-500/40 ":"")+(r%3===2&&r!==8?"border-b-2 border-b-emerald-500/40 ":"");
          return (
            <button key={`${r}-${c}`} type="button" onClick={()=>setSelected([r,c])}
              ...
              } ${isFixed?"text-emerald-200":"text-amber-200"}`}>{v||""}</button>
          );
        }))}
      </div>
      <div className="grid grid-cols-5 gap-2">
        {[1,2,3,4,5,6,7,8,9,0].map((n) => (
          <button key={n} type="button" onClick={()=>put(n)}
