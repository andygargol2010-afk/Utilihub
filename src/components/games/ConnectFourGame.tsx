...
/** Depth + randomness per difficulty. */
function cpuMove(board: Cell[][], level: CpuLevel): number {
  const legal = [3, 2, 4, 1, 5, 0, 6].filter((col) => !board[0]![col]);
  if (!legal.length) return 0;
  if (level === "easy") {
    if (Math.random() < 0.55) return legal[Math.floor(Math.random() * legal.length)]!;
    return minimax(board, 1, true, -Infinity, Infinity).col;
  }
  if (level === "medium") {
    if (Math.random() < 0.18) return legal[Math.floor(Math.random() * legal.length)]!;
    return minimax(board, 3, true, -Infinity, Infinity).col;
  }
  return minimax(board, 5, true, -Infinity, Infinity).col;
}
...
export function ConnectFourGame({ locale = "en" }: { locale?: GameLocale }) {
  ...
  const playCol = useCallback(
    (col: number) => {
      ...
      window.setTimeout(() => {
        setBoard((cur) => {
          ...
          const cpuCol = cpuMove(cur, level);
    ...
    [board, mode, result, turn, applyDrop, dropN, level],
  );
      ...
      <div className="flex flex-wrap justify-center gap-2">
        ...
        {mode === "cpu" && (
          <>
            ...
            <GameSecondaryButton active={level === "medium"} onClick={() => { setLevel("medium"); reset(); }}>
              {es ? "Medio" : "Med"}
            </GameSecondaryButton>
