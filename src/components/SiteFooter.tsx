...
export function SiteFooter() {
  const featured = ALL_CATEGORIES.filter((category) =>
    ["finanzas", "texto", "diseno", "seguridad"].includes(category.slug),
  );
  return (
    <footer className="mt-16 border-t border-border/70 bg-surface/55">
      <div className="container-page grid gap-10 py-10 sm:grid-cols-2 lg:grid-cols-4 sm:py-12">
        <div>
          <Link to="/" className="flex min-h-11 items-center gap-2 font-extrabold tracking-tight">
            <img src="/utilihub-logo.svg" alt="" className="brand-mark size-8" />
            UtiliHub
          </Link>
          <p className="mt-3 max-w-sm text-sm leading-6 text-muted-foreground">
            Free online tools to calculate, convert, and get things done without the hassle.
          </p>
          <p className="mt-4 text-xs font-semibold uppercase tracking-[.16em] text-primary">No signup · Local privacy</p>
        </div>
        <div>
          <p className="text-xs font-bold uppercase tracking-[.16em] text-muted-foreground">Explore</p>
          <nav aria-label="Featured categories" className="mt-3 grid gap-2">
        ...
        <div>
          <p className="text-xs font-bold uppercase tracking-[.16em] text-muted-foreground">Games</p>
          <nav aria-label="Games" className="mt-3 grid gap-2">
            <Link to="/games" className="w-fit text-sm font-semibold hover:text-primary">
              All games
            </Link>
            {GAMES.map((game) => (
              <a key={game.slug} href={gamePath(game, "en")} className="w-fit text-sm font-semibold hover:text-primary">
                {gameName(game, "en")}
              </a>
            ))}
          </nav>
      ...
      <div className="container-page flex flex-col gap-2 border-t border-border/70 py-5 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
        ...
        <span>Built to solve problems, not create them.</span>
      </div>
    </footer>
  );
}
