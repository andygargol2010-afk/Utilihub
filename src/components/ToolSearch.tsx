...
export function ToolSearch({ initialCategory, compactHome = false }: { initialCategory?: string; compactHome?: boolean }) {
  ...
  const compactResults = query.trim() ? results.slice(0, 6) : [];
  const submitCompactSearch = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (compactResults[0]) window.location.assign(toolHref(compactResults[0])); };
  ...
  return <div className="space-y-5">
    <div className="rounded-xl border border-border/70 bg-surface/45 p-2.5 sm:p-3">
      <form className="relative" onSubmit={compactHome ? submitCompactSearch : undefined}><Search className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar una herramienta…" aria-label="Buscar herramientas por nombre, descripción o palabra clave" className="h-12 rounded-lg border-transparent bg-card pl-11 text-base shadow-none focus-visible:border-primary/40" />{compactHome && compactResults.length > 0 && <div className="absolute inset-x-0 top-[calc(100%+0.5rem)] z-30 overflow-hidden rounded-xl border border-border bg-card p-1.5 shadow-lift" role="listbox" aria-label="Resultados de búsqueda">{compactResults.map((tool) => <Link key={tool.slug} to={toolHref(tool)} role="option" className="block rounded-lg px-3 py-2.5 hover:bg-accent"><span className="block text-sm font-bold">{tool.name}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{tool.summary}</span></Link>)}</div>}{compactHome && query.trim() && compactResults.length === 0 && <div className="absolute inset-x-0 top-[calc(100%+0.5rem)] z-30 rounded-xl border border-border bg-card p-4 text-center text-sm text-muted-foreground shadow-lift" role="status">No encontramos herramientas para «{query}».</div>}</form>
