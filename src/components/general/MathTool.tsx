...
function calculate(slug: string, raw: string, rawB: string): string {
  ...
  switch (slug) {
    ...
    case "cuartiles": { const a = [...xs].sort((x, y) => x - y); const q2 = median(a); const mid = Math.floor(a.length / 2); const q1 = median(a.slice(0, mid)); const q3 = median(a.slice(a.length % 2 ? mid + 1 : mid)); return `Q1: ${q1}\nMediana (Q2): ${q2}\nQ3: ${q3}`; }
    ...
    case "correlacion":
    case "covarianza": { const ys = values(rawB); if (ys.length !== xs.length || ys.length < 2) throw new Error("Las dos series deben tener la misma cantidad de valores (mínimo 2)."); const cov = xs.reduce((s, x, i) => s + (x - mean) * (ys[i] - ys.reduce((a, b) => a + b, 0) / ys.length), 0) / xs.length; if (slug === "covarianza") return `Covarianza poblacional: ${cov}`; const sdX = Math.sqrt(xs.reduce((s, x) => s + (x - mean) ** 2, 0) / xs.length); const meanY = ys.reduce((a, b) => a + b, 0) / ys.length; const sdY = Math.sqrt(ys.reduce((s, y) => s + (y - meanY) ** 2, 0) / ys.length); if (sdX === 0 || sdY === 0) throw new Error("La correlación requiere variación en ambas series."); return `Correlación de Pearson: ${cov / (sdX * sdY)}`; }
