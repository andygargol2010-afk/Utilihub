    ...
    <div className="space-y-4">
      {tool.slug === "generador-numeros" && (
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="space-y-1 text-sm font-medium">Mínimo<input type="number" value={min} onChange={(event) => setMin(event.target.value)} className="mt-1 h-11 w-full rounded-xl border bg-background px-3" /></label>
          <label className="space-y-1 text-sm font-medium">Máximo<input type="number" value={max} onChange={(event) => setMax(event.target.value)} className="mt-1 h-11 w-full rounded-xl border bg-background px-3" /></label>
        </div>
      )}
      {tool.slug === "generador-dados" && (
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="space-y-1 text-sm font-medium">Caras por dado<input type="number" min="2" max="10000" value={sides} onChange={(event) => setSides(event.target.value)} className="mt-1 h-11 w-full rounded-xl border bg-background px-3" /></label>
          <label className="space-y-1 text-sm font-medium">Cantidad de dados<input type="number" min="1" max="1000" value={count} onChange={(event) => setCount(event.target.value)} className="mt-1 h-11 w-full rounded-xl border bg-background px-3" /></label>
        </div>
      )}
