...
const FIELD_MAP: Record<string, Field[]> = {
  densidad: [
    { label: "Masa", unit: "kg", placeholder: "Ej.: 2" },
    { label: "Volumen", unit: "m³", placeholder: "Ej.: 0.5" },
  ],
  "velocidad-fisica": [
    { label: "Distancia", unit: "m", placeholder: "Ej.: 100" },
    { label: "Tiempo", unit: "s", placeholder: "Ej.: 20" },
  ],
  aceleracion: [
    { label: "Cambio de velocidad", unit: "m/s", placeholder: "Ej.: 20" },
    { label: "Tiempo", unit: "s", placeholder: "Ej.: 4" },
  ],
  fuerza: [
    { label: "Masa", unit: "kg", placeholder: "Ej.: 10" },
    { label: "Aceleración", unit: "m/s²", placeholder: "Ej.: 9.81" },
  ],
  "energia-cinetica": [
    { label: "Masa", unit: "kg", placeholder: "Ej.: 10" },
    { label: "Velocidad", unit: "m/s", placeholder: "Ej.: 5" },
  ],
  "energia-potencial": [
    { label: "Masa", unit: "kg", placeholder: "Ej.: 10" },
    { label: "Altura", unit: "m", placeholder: "Ej.: 5" },
  ],
  "ley-de-ohm": [
    { label: "Corriente", unit: "A", placeholder: "Ej.: 2" },
    { label: "Resistencia", unit: "Ω", placeholder: "Ej.: 10" },
  ],
  "potencia-electrica": [
    { label: "Voltaje", unit: "V", placeholder: "Ej.: 230" },
    { label: "Corriente", unit: "A", placeholder: "Ej.: 2" },
  ],
  "resistencias-serie": [
    { label: "Resistencia 1", unit: "Ω", placeholder: "Ej.: 10" },
    { label: "Resistencia 2", unit: "Ω", placeholder: "Ej.: 20" },
  ],
  ...
  "longitud-onda": [
    { label: "Velocidad de onda", unit: "m/s", placeholder: "Ej.: 343" },
    { label: "Frecuencia", unit: "Hz", placeholder: "Ej.: 440" },
  ],
  presion: [
    { label: "Fuerza", unit: "N", placeholder: "Ej.: 100" },
    { label: "Área", unit: "m²", placeholder: "Ej.: 2" },
  ],
  caudal: [
    { label: "Volumen", unit: "m³", placeholder: "Ej.: 1" },
    { label: "Tiempo", unit: "s", placeholder: "Ej.: 10" },
  ],
...
export function ScienceTool({ tool }: { tool: GeneralTool }) {
  ...
  const run = () => {
    try {
      ...
      const b = fields.length > 1 ? Number(second) : 0;
      if (!Number.isFinite(a) || (fields.length > 1 && !Number.isFinite(b))) {
        throw new Error("Introduce todos los valores requeridos con números válidos.");
      }
      if (POSITIVE_INPUT_SLUGS.has(tool.slug) && a <= 0) {
        throw new Error(`${fields[0].label} debe ser mayor que cero.`);
      }
      if (fields.length > 1 && POSITIVE_INPUT_SLUGS.has(tool.slug) && b <= 0) {
        throw new Error(`${fields[1].label} debe ser mayor que cero.`);
      }
      if (DIVISOR_SLUGS.has(tool.slug) && b === 0) {
        throw new Error(`${fields[1].label} no puede ser cero.`);
      }
      setOut(calculate(tool.slug, a, b));
    } catch (error) {
  ...
  if (!fields) {
    return <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm">Herramienta científica no configurada: {tool.slug}</div>;
  }
  ...
  return (
    <div className="space-y-4">
          ...
          <label key={field.label} className="space-y-1">
            ...
            <div className="flex">
              <input
                ...
                onChange={(event) => index === 0 ? setFirst(event.target.value) : setSecond(event.target.value)}
                placeholder={field.placeholder}
                type="number"
                inputMode="decimal"
                className="h-11 min-w-0 flex-1 rounded-l-xl border bg-background px-3"
              />
      ...
      <p className="text-xs text-muted-foreground">
        Las magnitudes se interpretan en las unidades indicadas y las entradas físicas no pueden ser negativas cuando la fórmula requiere una magnitud positiva.
      </p>
