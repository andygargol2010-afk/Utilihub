import { useState } from "react";
import type { GeneralTool } from "@/lib/general/types";
import { getToolShowcase } from "@/lib/tool-showcase";

const units: Record<string, Record<string, number>> = {
  area: { "m²": 1, "km²": 1e6, "ft²": 0.092903, acres: 4046.8564224, ha: 10000 },
  volume: { L: 1, mL: 0.001, gal: 3.785411784, "m³": 1000 },
  speed: { "km/h": 1, mph: 1.609344, "m/s": 3.6, kn: 1.852 },
  energy: { J: 1, cal: 4.184, kWh: 3600000, BTU: 1055.05585262 },
  power: { W: 1, kW: 1000, hp: 745.699872 },
  pressure: { Pa: 1, bar: 100000, atm: 101325, psi: 6894.757293 },
  frequency: { Hz: 1, kHz: 1000, MHz: 1e6, GHz: 1e9 },
  data: { bit: 1, byte: 8, KB: 8000, MB: 8e6, GB: 8e9, TB: 8e12 },
};
const sets: Record<string, string[]> = {
  area: ["m²", "km²", "ft²", "acres", "ha"],
  volume: ["L", "mL", "gal", "m³"],
  speed: ["km/h", "mph", "m/s", "kn"],
  energy: ["J", "cal", "kWh", "BTU"],
  power: ["W", "kW", "hp"],
  pressure: ["Pa", "bar", "atm", "psi"],
  frequency: ["Hz", "kHz", "MHz", "GHz"],
  data: ["bit", "byte", "KB", "MB", "GB", "TB"],
};
const hexToRgb = (v: string) => {
  const h = v.replace(/^#/, "");
  if (!/^[0-9a-f]{6}$/i.test(h)) return null;
  return { r: parseInt(h.slice(0, 2), 16), g: parseInt(h.slice(2, 4), 16), b: parseInt(h.slice(4, 6), 16) };
};
const rgbToHsl = (r: number, g: number, b: number) => {
  r /= 255;
  g /= 255;
  b /= 255;
  const max = Math.max(r, g, b),
    min = Math.min(r, g, b),
    d = max - min;
  let h = 0,
    s = 0;
  const l = (max + min) / 2;
  if (d) {
    s = d / (1 - Math.abs(2 * l - 1));
    switch (max) {
      case r:
        h = ((g - b) / d + (g < b ? 6 : 0)) / 6;
        break;
      case g:
        h = ((b - r) / d + 2) / 6;
        break;
      default:
        h = ((r - g) / d + 4) / 6;
    }
  }
  return { h: h * 360, s: s * 100, l: l * 100 };
};

export function ConverterTool({ tool, locale = "en" }: { tool: GeneralTool; locale?: "en" | "es" }) {
  const [value, setValue] = useState("");
  const [from, setFrom] = useState("");
  const [to, setTo] = useState("");
  const [out, setOut] = useState("");
  const es = locale === "es";
  const premium = Boolean(getToolShowcase(tool.slug));
  const base = tool.slug.replace("conversor-", "");
  const kind =
    base === "datos"
      ? "data"
      : base === "velocidad"
        ? "speed"
        : base === "energia"
          ? "energy"
          : base === "potencia"
            ? "power"
            : base === "presion"
              ? "pressure"
              : base === "frecuencia"
                ? "frequency"
                : base === "area"
                  ? "area"
                  : base === "volumen"
                    ? "volume"
                    : "";
  const options = sets[kind] ?? [];
  const msg = (en: string, esText: string) => (es ? esText : en);

  const convert = () => {
    const raw = value.trim();
    if (tool.slug === "binario-decimal") {
      if (!/^[01]+$/.test(raw)) {
        setOut(msg("Enter a valid binary number.", "Introduce un número binario válido."));
        return;
      }
      setOut(`${parseInt(raw, 2)}`);
      return;
    }
    if (tool.slug === "decimal-binario") {
      const n = Number(raw);
      if (!Number.isInteger(n) || n < 0) {
        setOut(msg("Enter a non-negative decimal integer.", "Introduce un entero decimal no negativo."));
        return;
      }
      setOut(n.toString(2));
      return;
    }
    if (tool.slug === "hexadecimal-decimal") {
      if (!/^(?:0x)?[0-9a-f]+$/i.test(raw)) {
        setOut(msg("Enter valid hexadecimal.", "Introduce un valor hexadecimal válido."));
        return;
      }
      setOut(`${parseInt(raw.replace(/^0x/i, ""), 16)}`);
      return;
    }
    if (tool.slug === "decimal-hexadecimal") {
      const n = Number(raw);
      if (!Number.isInteger(n) || n < 0) {
        setOut(msg("Enter a non-negative decimal integer.", "Introduce un entero decimal no negativo."));
        return;
      }
      setOut(n.toString(16).toUpperCase());
      return;
    }
    if (tool.slug === "conversor-angulos" || tool.slug === "grados-radianes") {
      const n = Number(raw.replace(",", "."));
      if (!Number.isFinite(n)) {
        setOut(msg("Enter a valid angle.", "Introduce un ángulo válido."));
        return;
      }
      const a = from || "degrees",
        b = to || "radians";
      setOut(
        a === b
          ? `${n} ${a}`
          : a === "degrees" || a === "grados"
            ? `${n} deg = ${((n * Math.PI) / 180).toFixed(10)} rad`
            : `${n} rad = ${((n * 180) / Math.PI).toFixed(10)} deg`,
      );
      return;
    }
    if (tool.slug === "rgb-hex") {
      const p = raw.split(/[,; ]+/).map(Number);
      if (p.length !== 3 || p.some((x) => !Number.isInteger(x) || x < 0 || x > 255)) {
        setOut(msg("Enter R, G and B between 0 and 255.", "Introduce R, G y B entre 0 y 255."));
        return;
      }
      setOut(`#${p.map((x) => x.toString(16).padStart(2, "0")).join("").toUpperCase()}`);
      return;
    }
    if (tool.slug === "hex-rgb") {
      const rgb = hexToRgb(raw);
      if (!rgb) {
        setOut(msg("Enter a valid HEX, for example #2563EB.", "Introduce un HEX válido, por ejemplo #2563EB."));
        return;
      }
      setOut(`RGB: ${rgb.r}, ${rgb.g}, ${rgb.b}`);
      return;
    }
    if (tool.slug === "rgb-hsl") {
      const p = raw.split(/[,; ]+/).map(Number);
      if (p.length !== 3 || p.some((x) => !Number.isInteger(x) || x < 0 || x > 255)) {
        setOut(msg("Enter R, G and B between 0 and 255.", "Introduce R, G y B entre 0 y 255."));
        return;
      }
      const h = rgbToHsl(p[0], p[1], p[2]);
      setOut(`HSL: ${h.h.toFixed(1)}°, ${h.s.toFixed(1)}%, ${h.l.toFixed(1)}%`);
      return;
    }
    const n = Number(raw.replace(",", "."));
    if (!Number.isFinite(n)) {
      setOut(msg("Enter a valid number.", "Introduce un número válido."));
      return;
    }
    if (kind) {
      const a = from || options[0],
        b = to || options[1] || options[0];
      setOut(`${n} ${a} = ${((n * units[kind][a]) / units[kind][b]).toPrecision(10)} ${b}`);
      return;
    }
    setOut(msg("Enter the data to convert.", "Introduce los datos que quieres convertir."));
  };

  const angleOptions = ["degrees", "radians"];
  const inputClass = premium
    ? "h-12 w-full rounded-2xl border-2 border-sky-200/80 bg-[linear-gradient(180deg,#f8fbff_0%,#ffffff_60%)] px-4 shadow-inner transition focus:border-sky-400 focus:outline-none focus:ring-4 focus:ring-sky-100"
    : "h-11 w-full rounded-xl border bg-background px-3";
  const selectClass = premium
    ? "h-12 rounded-2xl border-2 border-sky-200/80 bg-white px-3 shadow-sm"
    : "h-11 rounded-xl border bg-background px-3";
  const btnClass = premium
    ? "inline-flex min-h-12 items-center justify-center rounded-full bg-gradient-to-r from-sky-500 to-blue-600 px-8 py-3.5 text-sm font-bold text-white shadow-[0_12px_28px_-10px_rgba(37,99,235,0.55)] transition hover:brightness-105"
    : "rounded-xl bg-primary px-4 py-2 font-bold text-primary-foreground";

  return (
    <div className="space-y-4">
      <label className="block space-y-1">
        <span className="text-sm font-medium">{msg("Value", "Valor")}</span>
        <input
          value={value}
          onChange={(e) => setValue(e.target.value)}
          className={inputClass}
          placeholder={tool.slug.includes("rgb") ? "255, 99, 71" : "100"}
        />
      </label>
      {(options.length > 1 || tool.slug === "conversor-angulos" || tool.slug === "grados-radianes") && (
        <div className="grid gap-4 sm:grid-cols-2">
          <select
            aria-label={msg("From unit", "Unidad de origen")}
            value={from}
            onChange={(e) => setFrom(e.target.value)}
            className={selectClass}
          >
            {(tool.slug.includes("angulos") || tool.slug === "grados-radianes" ? angleOptions : options).map((x) => (
              <option key={x}>{x}</option>
            ))}
          </select>
          <select
            aria-label={msg("To unit", "Unidad de destino")}
            value={to}
            onChange={(e) => setTo(e.target.value)}
            className={selectClass}
          >
            {(tool.slug.includes("angulos") || tool.slug === "grados-radianes" ? angleOptions : options).map((x) => (
              <option key={x}>{x}</option>
            ))}
          </select>
        </div>
      )}
      <button type="button" className={btnClass} onClick={convert}>
        {msg("Convert", "Convertir")}
      </button>
      {out && (
        <output className={`block break-all rounded-2xl border p-4 ${premium ? "border-sky-100 bg-sky-50/50 font-medium" : "bg-muted/30"}`}>
          {out}
        </output>
      )}
    </div>
  );
}
