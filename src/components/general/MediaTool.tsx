...
const rotatedBounds = (width: number, height: number, angle: number) => {
  const r = Math.abs(angle % 360) * Math.PI / 180;
  return {
    width: Math.ceil(Math.abs(width * Math.cos(r)) + Math.abs(height * Math.sin(r))),
    height: Math.ceil(Math.abs(width * Math.sin(r)) + Math.abs(height * Math.cos(r))),
  };
};
...
export function MediaTool({ tool }: { tool: GeneralTool }) {
  ...
  useEffect(() => {
    ...
    if (file.type.startsWith("image/")) {
      const img = new Image();
      img.onload = () => setInfo(`${img.width} × ${img.height}px · ${(file.size / 1024).toFixed(1)} KB`);
      img.src = u;
    } else {
      setInfo(`${(file.size / 1024).toFixed(1)} KB · ${file.type || "PDF"}`);
    }
    ...
    try {
      ...
      if (image) {
        ...
        if (tool.slug === "recortar-imagen") {
          ...
          h = Math.min(img.height, Math.max(1, Math.round(Number(height) || img.height)));
        }
        ...
        const type = tool.slug.endsWith("-jpg") || tool.slug === "comprimir-imagen"
          ? "image/jpeg"
          : tool.slug.endsWith("-webp") ? "image/webp" : "image/png";
        if (type === "image/jpeg") {
          ctx.save();
  ...
  return (
    <div className="space-y-4">
      ...
      {out && <pre className="max-h-96 overflow-auto whitespace-pre-wrap rounded-xl border bg-muted/30 p-4 text-sm">{out}</pre>}
    </div>
  );
}
