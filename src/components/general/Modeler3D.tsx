...
export function Modeler3D({ locale = "en" }: { tool: GeneralTool; locale?: "en" | "es" }) {
  ...
  const undo = useCallback(() => {
    ...
    rebuildFromSnapshot(prev, selectedIdRef.current);
  ...
  const redo = useCallback(() => {
    ...
    setCanRedo(futureRef.current.length > 0);
    skipHistoryRef.current = true;
    rebuildFromSnapshot(next, selectedIdRef.current);
    skipHistoryRef.current = false;
  }, [captureSnapshot, rebuildFromSnapshot]);
  ...
  const exportStl = useCallback(() => {
    ...
    for (const o of objectsRef.current) {
      ...
      const writeTri = (ax: number, ay: number, az: number, bx: number, by: number, bz: number, cx: number, cy: number, cz: number) => {
        ...
        body += `    outer loop\n`;
        body += `      vertex ${ax} ${ay} ${az}\n`;
        body += `      vertex ${bx} ${by} ${bz}\n`;
        body += `      vertex ${cx} ${cy} ${cz}\n`;
        body += `    endloop\n`;
        body += `  endfacet\n`;
      };
      if (idx) {
    ...
    const stl = `solid utilihub\n${body}endsolid utilihub\n`;
    const blob = new Blob([stl], { type: "model/stl" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "utilihub-model.stl";
    a.click();
    URL.revokeObjectURL(a.href);
  }, []);
    ...
    >
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-rose-500/20 bg-black/30 px-3 py-2.5 sm:px-4">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <button type="button" onClick={undo} disabled={!canUndo} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold text-rose-100/90 hover:bg-white/10 disabled:opacity-40" title="Ctrl+Z">{es ? "Deshacer" : "Undo"}</button>
          <button type="button" onClick={redo} disabled={!canRedo} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold text-rose-100/90 hover:bg-white/10 disabled:opacity-40" title="Ctrl+Y">{es ? "Rehacer" : "Redo"}</button>
          ...
          <button type="button" onClick={exportJson} disabled={!ready || objects.length === 0} className="rounded-full border border-rose-400/30 bg-rose-500/15 px-3 py-1.5 text-xs font-semibold text-rose-100 hover:bg-rose-500/25 disabled:opacity-40">JSON</button>
          ...
          <div ref={mountRef} className={`relative min-h-0 w-full flex-1 overflow-hidden rounded-xl border border-rose-500/20 bg-[#0c0810] ${fullscreen ? "min-h-[200px]" : "aspect-[5/3.4] min-h-[280px] max-h-[520px] sm:min-h-[340px]"}`}>
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex flex-wrap items-end justify-between gap-2 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-2.5 pt-8">
              <div className="pointer-events-auto flex flex-wrap items-center gap-1.5">
                ...
                <button type="button" onClick={duplicateSelected} disabled={!selectedId} className="rounded-full border border-white/15 bg-black/50 px-3 py-1.5 text-xs font-semibold text-rose-100/90 backdrop-blur-sm hover:bg-white/15 disabled:opacity-40" title="Ctrl+D">{es ? "Duplicar" : "Duplicate"}</button>
                <button type="button" onClick={deleteSelected} disabled={!selectedId} className="rounded-full border border-rose-500/40 bg-rose-950/70 px-3 py-1.5 text-xs font-semibold text-rose-200 backdrop-blur-sm hover:bg-rose-900/80 disabled:opacity-40">{es ? "Eliminar" : "Delete"}</button>
              ...
              {objects.map((o) => (
                <li key={o.id}>
                  {editingNameId === o.id ? (
                    ...
                    <button type="button" onClick={() => selectMesh(o.id)} onDoubleClick={() => { setEditingNameId(o.id); setEditingName(o.name); }} className={`flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-xs font-medium transition ${selectedId === o.id ? "bg-rose-500/25 text-rose-50 ring-1 ring-rose-400/40" : "text-rose-100/80 hover:bg-white/5"}`} title={es ? "Doble click para renombrar" : "Double-click to rename"}>
            ...
            <div className="mt-3 space-y-3 border-t border-rose-500/15 pt-3">
                ...
                {([0, 1, 2] as const).map((axis) => {
                  ...
                  return (
                    <div key={`pos-${axis}`} className="flex items-center justify-between gap-1">
                      ...
                      <input type="number" step={snap > 0 ? snap : 0.05} value={Number(pos[axis].toFixed(3))} onChange={(e) => setPosAxis(axis, parseFloat(e.target.value) || 0)} onBlur={endTransformEdit} className="w-20 rounded border border-white/10 bg-black/40 px-1.5 py-1 text-center text-xs font-semibold text-rose-50 outline-none focus:border-rose-400/50" />
                    </div>
                  );
                })}
              </div>
                ...
                {([0, 1, 2] as const).map((axis) => {
                  ...
                  return (
                    <div key={`rot-${axis}`} className="flex items-center justify-between gap-1">
                      <span className="w-4 text-[10px] font-bold" style={{ color: cols[axis] }}>{labels[axis]}</span>
                      <input type="number" step={15} value={Number(rotDeg[axis].toFixed(1))} onChange={(e) => setRotAxis(axis, parseFloat(e.target.value) || 0)} onBlur={endTransformEdit} className="w-20 rounded border border-white/10 bg-black/40 px-1.5 py-1 text-center text-xs font-semibold text-rose-50 outline-none focus:border-rose-400/50" />
                    </div>
                  );
                })}
              </div>
              <div className="space-y-2 border-t border-rose-500/10 pt-2">
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  <button type="button" onClick={() => { const meta = objects.find((o) => o.id === selectedId); if (meta) applySize([...BASE_SIZE[meta.kind]]); }} className="rounded-full border border-white/15 bg-white/5 px-2 py-1 text-[10px] font-bold">Reset</button>
                </div>
              </div>
            </div>
          )}
