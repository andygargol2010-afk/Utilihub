...
export function Modeler3D({ locale = "en" }: { tool: GeneralTool; locale?: "en" | "es" }) {
  ...
  const captureSnapshot = useCallback((): MeshSnapshot[] => {
    ...
    return objectsRef.current.map((o) => {
      ...
      return { id: o.id, name: o.name, kind: o.kind, color: `#${mesh.material.color.getHexString()}`, position: mesh.position.toArray() as [number, number, number], rotation: [mesh.rotation.x, mesh.rotation.y, mesh.rotation.z] as [number, number, number], scale: mesh.scale.toArray() as [number, number, number] };
    });
  }, []);
  ...
  const selectMesh = useCallback((id: string | null) => {
    ...
    if (!t) return;
    if (id && t.meshes.has(id)) { t.transform.attach(t.meshes.get(id)); t.transform.setMode(modeRef.current); readTransform(id); }
    else { t.transform.detach(); setSize([1, 1, 1]); setPos([0, 0.55, 0]); setRotDeg([0, 0, 0]); }
  }, [readTransform]);
  ...
  const deleteSelected = useCallback(() => {
    ...
    pushHistory();
    const mesh = t.meshes.get(id);
    if (mesh) { t.transform.detach(); t.scene.remove(mesh); mesh.geometry?.dispose(); mesh.material?.dispose(); t.meshes.delete(id); }
  ...
  const clearScene = useCallback(() => {
    ...
    t.transform.detach();
    for (const [, mesh] of t.meshes) { t.scene.remove(mesh); mesh.geometry?.dispose(); mesh.material?.dispose(); }
    t.meshes.clear(); setObjects([]); objectsRef.current = []; selectMesh(null);
  }, [pushHistory, selectMesh]);
  ...
  const exportStl = useCallback(() => {
    ...
    for (const o of objectsRef.current) {
      const mesh = t.meshes.get(o.id); if (!mesh) continue;
  ...
  useEffect(() => { (window as any).__utilihubSnap = snap; }, [snap]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const accent = new THREE.PointLight(0xf43f5e, 0.55, 14, 2); accent.position.set(-1.5, 2.2, 2.5); scene.add(accent);
        ...
        const shadowCatcher = new THREE.Mesh(new THREE.PlaneGeometry(12, 12), new THREE.ShadowMaterial({ opacity: 0.45 }));
          ...
          if (!event.value && selectedIdRef.current) {
            ...
            if (mesh && meta) {
              ...
              if (step > 0 && modeRef.current === "translate") {
                mesh.position.x = snapVal(mesh.position.x, step); mesh.position.y = snapVal(mesh.position.y, step); mesh.position.z = snapVal(mesh.position.z, step);
              }
        ...
        const onPointer = (event: PointerEvent) => {
          ...
          if (hits.length > 0) {
            const id = hits[0]!.object.userData.id as string; const obj = hits[0]!.object;
            selectedIdRef.current = id; setSelectedId(id); transform.attach(obj); transform.setMode(modeRef.current);
            const kind = obj.userData.kind as ShapeKind; const b = BASE_SIZE[kind];
            setSize([clampSize(b[0] * obj.scale.x), clampSize(b[1] * obj.scale.y), clampSize(b[2] * obj.scale.z)]);
            setPos([Math.round(obj.position.x * 1000) / 1000, Math.round(obj.position.y * 1000) / 1000, Math.round(obj.position.z * 1000) / 1000]);
            setRotDeg([Math.round(radToDeg(obj.rotation.x) * 10) / 10, Math.round(radToDeg(obj.rotation.y) * 10) / 10, Math.round(radToDeg(obj.rotation.z) * 10) / 10]);
          } else {
  ...
  return (
    <div ref={studioRef} className={`-mx-1 flex flex-col overflow-hidden rounded-[1.5rem] border border-rose-400/25 bg-gradient-to-b from-[#1a0f18] via-[#120a12] to-[#0a060c] text-rose-50 shadow-[0_28px_70px_-24px_rgba(244,63,94,0.45),0_0_0_1px_rgba(244,63,94,0.12)] ${fullscreen ? "fixed inset-0 z-50 m-0 h-full max-h-[100dvh] rounded-none border-0" : ""}`}>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-0 lg:flex-row">
        <aside className="flex shrink-0 flex-row gap-2 overflow-x-auto border-b border-rose-500/15 bg-black/20 p-2.5 lg:w-36 lg:flex-col lg:overflow-y-auto lg:border-b-0 lg:border-r">
          ...
          <p className="hidden text-[10px] font-bold uppercase tracking-wider text-rose-300/70 lg:block">Color</p>
        ...
        <aside className="max-h-[42vh] shrink-0 overflow-y-auto border-t border-rose-500/15 bg-black/25 p-2.5 lg:max-h-none lg:w-52 lg:border-l lg:border-t-0">
          ...
          {objects.length === 0 ? (<p className="text-[11px] leading-relaxed text-rose-200/45">{es ? "Agregá una forma desde la barra." : "Add a shape from the bar."}</p>) : (
            ...
            <div className="mt-3 space-y-3 border-t border-rose-500/15 pt-3">
              ...
              <div className="space-y-2 border-t border-rose-500/10 pt-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {[0.5, 1.5, 2].map((f) => (<button key={f} type="button" onClick={() => applySize([size[0] * f, size[1] * f, size[2] * f])} className="rounded-full border border-rose-400/25 bg-rose-950/50 px-2 py-1 text-[10px] font-bold">{f}x</button>))}
