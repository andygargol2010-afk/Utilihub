...
const BASE_SIZE: Record<ShapeKind, [number, number, number]> = {
  box: [1, 1, 1], sphere: [1.1, 1.1, 1.1], cylinder: [0.9, 1.1, 0.9], cone: [1, 1.1, 1], plane: [1.4, 0.06, 1.4],
};
...
function snapVal(n: number, step: number) {
...
export function Modeler3D({ locale = "en" }: { tool: GeneralTool; locale?: "en" | "es" }) {
  ...
  const readTransform = useCallback((id: string | null) => {
    ...
    setRotDeg([
      Math.round(radToDeg(mesh.rotation.x) * 10) / 10,
      Math.round(radToDeg(mesh.rotation.y) * 10) / 10,
      Math.round(radToDeg(mesh.rotation.z) * 10) / 10,
    ]);
  }, []);
  ...
  const setPosAxis = useCallback((axis: 0 | 1 | 2, value: number) => {
    ...
    const mesh = t.meshes.get(id);
    if (!mesh) return;
    if (!transformHistPushedRef.current) {
      pushHistory();
      transformHistPushedRef.current = true;
    }
    ...
    setPos([
      Math.round(next[0] * 1000) / 1000,
      Math.round(next[1] * 1000) / 1000,
      Math.round(next[2] * 1000) / 1000,
    ]);
  }, [pushHistory, snap]);
  ...
  const setRotAxis = useCallback((axis: 0 | 1 | 2, value: number) => {
    ...
    setRotDeg([
      Math.round(next[0] * 10) / 10,
      Math.round(next[1] * 10) / 10,
      Math.round(next[2] * 10) / 10,
    ]);
  }, [pushHistory]);
  ...
  const duplicateSelected = useCallback(() => {
    ...
    createMesh(meta.kind, {
      color: `#${mesh.material.color.getHexString()}`,
      name: `${meta.name} ${es ? "copia" : "copy"}`,
      position: [mesh.position.x + offset, mesh.position.y, mesh.position.z + offset],
      rotation: [mesh.rotation.x, mesh.rotation.y, mesh.rotation.z],
      scale: mesh.scale.toArray() as [number, number, number],
    });
  }, [createMesh, es, snap]);
  ...
  const renameObject = useCallback((id: string, name: string) => {
    ...
    setObjects((prev) => {
      const next = prev.map((o) => (o.id === id ? { ...o, name: trimmed } : o));
      objectsRef.current = next;
      return next;
    });
  ...
  useEffect(() => {
    (window as any).__utilihubSnap = snap;
  }, [snap]);
    ...
    (async () => {
      try {
        ...
        transform.addEventListener("dragging-changed", (event: { value: boolean }) => {
          ...
          if (!event.value && selectedIdRef.current) {
            const mesh = meshes.get(selectedIdRef.current);
            const meta = objectsRef.current.find((o) => o.id === selectedIdRef.current);
            if (mesh && meta) {
              const step = snapRef.current || 0;
              if (step > 0 && modeRef.current === "translate") {
                mesh.position.x = snapVal(mesh.position.x, step);
                mesh.position.y = snapVal(mesh.position.y, step);
                mesh.position.z = snapVal(mesh.position.z, step);
              }
              if (step > 0 && modeRef.current === "rotate") {
                const snapDeg = 15;
                mesh.rotation.x = degToRad(snapVal(radToDeg(mesh.rotation.x), snapDeg));
                mesh.rotation.y = degToRad(snapVal(radToDeg(mesh.rotation.y), snapDeg));
                mesh.rotation.z = degToRad(snapVal(radToDeg(mesh.rotation.z), snapDeg));
              }
              const b = BASE_SIZE[meta.kind as ShapeKind];
              setSize([clampSize(b[0] * mesh.scale.x), clampSize(b[1] * mesh.scale.y), clampSize(b[2] * mesh.scale.z)]);
              setPos([
                Math.round(mesh.position.x * 1000) / 1000,
                Math.round(mesh.position.y * 1000) / 1000,
        ...
        const onPointer = (event: PointerEvent) => {
          ...
          if (hits.length > 0) {
            ...
            const kind = obj.userData.kind as ShapeKind;
            const b = BASE_SIZE[kind];
            setSize([clampSize(b[0] * obj.scale.x), clampSize(b[1] * obj.scale.y), clampSize(b[2] * obj.scale.z)]);
            setPos([
            ...
            setRotDeg([
              Math.round(radToDeg(obj.rotation.x) * 10) / 10,
              Math.round(radToDeg(obj.rotation.y) * 10) / 10,
              Math.round(radToDeg(obj.rotation.z) * 10) / 10,
            ]);
          } else {
            selectedIdRef.current = null;
            setSelectedId(null);
  ...
  }, [copySelected, deleteSelected, duplicateSelected, pasteClipboard, toggleFullscreen, undo]);
  ...
  return (
    <div
      ref={studioRef}
      className={`-mx-1 flex flex-col overflow-hidden rounded-[1.5rem] border border-rose-400/25 bg-gradient-to-b from-[#1a0f18] via-[#120a12] to-[#0a060c] text-rose-50 shadow-[0_28px_70px_-24px_rgba(244,63,94,0.45),0_0_0_1px_rgba(244,63,94,0.12)] ${fullscreen ? "fixed inset-0 z-50 m-0 h-full max-h-[100dvh] rounded-none border-0" : ""}`}
    >
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-rose-500/20 bg-black/30 px-3 py-2.5 sm:px-4">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <button type="button" onClick={undo} disabled={!canUndo} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold text-rose-100/90 hover:bg-white/10 disabled:opacity-40" title="Ctrl+Z">
            {es ? "Deshacer" : "Undo"}
          </button>
          <button type="button" onClick={exportJson} disabled={!ready || objects.length === 0} className="rounded-full border border-rose-400/30 bg-rose-500/15 px-3 py-1.5 text-xs font-semibold text-rose-100 hover:bg-rose-500/25 disabled:opacity-40">
            {es ? "Exportar JSON" : "Export JSON"}
          </button>
          <button type="button" onClick={clearScene} disabled={!ready || objects.length === 0} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold text-rose-100/90 hover:bg-white/10 disabled:opacity-40">
            {es ? "Limpiar" : "Clear"}
          </button>
          <button type="button" onClick={() => void toggleFullscreen()} className="rounded-full bg-gradient-to-b from-rose-300 to-rose-600 px-3 py-1.5 text-xs font-bold text-rose-950 shadow-md hover:from-rose-200 hover:to-rose-500">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-0 lg:flex-row">
        <aside className="flex shrink-0 flex-row gap-2 overflow-x-auto border-b border-rose-500/15 bg-black/20 p-2.5 lg:w-36 lg:flex-col lg:overflow-y-auto lg:border-b-0 lg:border-r">
          ...
          {shapes.map((s) => (
            <button key={s.kind} type="button" disabled={!ready} onClick={() => addShape(s.kind)} className="inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl border border-rose-400/20 bg-rose-950/40 px-3 text-xs font-semibold text-rose-50 transition hover:border-rose-400/50 hover:bg-rose-900/50 disabled:opacity-40 lg:w-full">
              <span className="text-base text-rose-300" aria-hidden>{s.icon}</span>
              {s.label}
            </button>
          ))}
          <div className="my-1 hidden h-px bg-rose-500/20 lg:block" />
          <p className="hidden text-[10px] font-bold uppercase tracking-wider text-rose-300/70 lg:block">{es ? "Color" : "Color"}</p>
          <div className="flex items-center gap-1.5 px-1">
            {COLORS.map((c) => (
              <button key={c} type="button" title={c} onClick={() => applyColor(c)} className={`size-6 rounded-full border-2 transition ${color === c ? "scale-110 border-white" : "border-transparent"}`} style={{ backgroundColor: c }} />
            ))}
            <input type="color" value={color} onChange={(e) => applyColor(e.target.value)} className="size-6 cursor-pointer rounded border-0 bg-transparent" title={es ? "Color personalizado" : "Custom color"} />
          </div>
        </aside>
        ...
        <div className="relative flex min-h-0 min-w-0 flex-1 flex-col p-2.5 sm:p-3">
          <div ref={mountRef} className={`relative min-h-0 w-full flex-1 overflow-hidden rounded-xl border border-rose-500/20 bg-[#0c0810] ${fullscreen ? "min-h-[200px]" : "aspect-[5/3.4] min-h-[280px] max-h-[520px] sm:min-h-[340px]"}`}>
            {!ready && !error && (
              <p className="absolute inset-0 flex items-center justify-center text-sm text-rose-200/60">{es ? "Cargando estudio 3D…" : "Loading 3D studio…"}</p>
            )}
            {error && (
              <p className="absolute inset-0 flex items-center justify-center px-4 text-center text-sm text-rose-300">{error}</p>
            )}
            <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex flex-wrap items-end justify-between gap-2 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-2.5 pt-8">
              <div className="pointer-events-auto flex flex-wrap items-center gap-1.5">
                {modes.map((m) => (
                  <button key={m.id} type="button" onClick={() => setMode(m.id)} className={mode === m.id ? "rounded-full bg-gradient-to-b from-rose-300 to-rose-600 px-3 py-1.5 text-xs font-bold text-rose-950 shadow" : "rounded-full border border-white/15 bg-black/50 px-3 py-1.5 text-xs font-semibold text-rose-100/90 backdrop-blur-sm hover:bg-white/15"}>
                    {m.label}
                  </button>
                ))}
                <button type="button" onClick={duplicateSelected} disabled={!selectedId} className="rounded-full border border-white/15 bg-black/50 px-3 py-1.5 text-xs font-semibold text-rose-100/90 backdrop-blur-sm hover:bg-white/15 disabled:opacity-40" title="Ctrl+D">
                  {es ? "Duplicar" : "Duplicate"}
                </button>
                <button type="button" onClick={deleteSelected} disabled={!selectedId} className="rounded-full border border-rose-500/40 bg-rose-950/70 px-3 py-1.5 text-xs font-semibold text-rose-200 backdrop-blur-sm hover:bg-rose-900/80 disabled:opacity-40">
                  {es ? "Eliminar" : "Delete"}
                </button>
              ...
              <p className="pointer-events-none hidden text-[10px] text-rose-200/60 sm:block">
                {es ? "Ctrl+C/V/D · Ctrl+Z · V/R/S · F" : "Ctrl+C/V/D · Ctrl+Z · V/R/S · F"}
              </p>
            </div>
          </div>
        </div>
        ...
        <aside className="max-h-[42vh] shrink-0 overflow-y-auto border-t border-rose-500/15 bg-black/25 p-2.5 lg:max-h-none lg:w-52 lg:border-l lg:border-t-0">
          ...
          {objects.length === 0 ? (
            <p className="text-[11px] leading-relaxed text-rose-200/45">{es ? "Agregá una forma desde la barra." : "Add a shape from the bar."}</p>
          ) : (
                ...
                <li key={o.id}>
                  {editingNameId === o.id ? (
                    <input
                      ...
                      onBlur={() => renameObject(o.id, editingName)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") renameObject(o.id, editingName);
                        if (e.key === "Escape") setEditingNameId(null);
                      }}
                      className="w-full rounded-lg border border-rose-400/40 bg-black/50 px-2 py-1.5 text-xs font-medium text-rose-50 outline-none"
                    />
                  ) : (
                    <button
                      type="button"
                      ...
                      onDoubleClick={() => {
                        setEditingNameId(o.id);
                        setEditingName(o.name);
                      }}
                      className={`flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-xs font-medium transition ${selectedId === o.id ? "bg-rose-500/25 text-rose-50 ring-1 ring-rose-400/40" : "text-rose-100/80 hover:bg-white/5"}`}
                      title={es ? "Doble click para renombrar" : "Double-click to rename"}
                    >
            ...
            <div className="mt-3 space-y-3 border-t border-rose-500/15 pt-3">
              <div className="space-y-1.5">
                ...
                {([0, 1, 2] as const).map((axis) => {
                  ...
                  return (
                    <div key={`pos-${axis}`} className="flex items-center justify-between gap-1">
                      <span className="w-4 text-[10px] font-bold" style={{ color: cols[axis] }}>{labels[axis]}</span>
                      <input
                        type="number"
                        step={snap > 0 ? snap : 0.05}
                        value={Number(pos[axis].toFixed(3))}
                        onChange={(e) => setPosAxis(axis, parseFloat(e.target.value) || 0)}
                        onBlur={endTransformEdit}
                        className="w-20 rounded border border-white/10 bg-black/40 px-1.5 py-1 text-center text-xs font-semibold text-rose-50 outline-none focus:border-rose-400/50"
                      />
                    </div>
                  );
                })}
              ...
              <div className="space-y-1.5">
                <p className="text-[10px] font-bold uppercase tracking-wider text-rose-300/70">{es ? "Rotación (°)" : "Rotation (°)"}</p>
                {([0, 1, 2] as const).map((axis) => {
                  const labels = ["X", "Y", "Z"];
                  const cols = ["#f87171", "#4ade80", "#60a5fa"];
                  return (
                    <div key={`rot-${axis}`} className="flex items-center justify-between gap-1">
                      <span className="w-4 text-[10px] font-bold" style={{ color: cols[axis] }}>{labels[axis]}</span>
                      <input
                        type="number"
                        step={15}
                        value={Number(rotDeg[axis].toFixed(1))}
              ...
              <div className="space-y-2 border-t border-rose-500/10 pt-2">
                <div className="flex items-center justify-between">
                  ...
                  <label className="flex items-center gap-1 text-[10px] font-semibold text-rose-200/80">
                    <input type="checkbox" checked={uniformScale} onChange={(e) => setUniformScale(e.target.checked)} className="size-3.5" />
                    {es ? "Uniforme" : "Uniform"}
                  </label>
                </div>
                {([0, 1, 2] as const).map((axis) => {
                ...
                <div className="flex flex-wrap gap-1">
                  {[0.5, 1.5, 2].map((f) => (
                    <button key={f} type="button" onClick={() => applySize([size[0] * f, size[1] * f, size[2] * f])} className="rounded-full border border-rose-400/25 bg-rose-950/50 px-2 py-1 text-[10px] font-bold">{f}x</button>
                  ))}
                  <button type="button" onClick={() => {
                    const meta = objects.find((o) => o.id === selectedId);
                    if (meta) applySize([...BASE_SIZE[meta.kind]]);
                  }} className="rounded-full border border-white/15 bg-white/5 px-2 py-1 text-[10px] font-bold">Reset</button>
                </div>
              </div>
            </div>
          )}
