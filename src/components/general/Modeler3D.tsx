...
export function Modeler3D({ locale = "en" }: { tool: GeneralTool; locale?: "en" | "es" }) {
  ...
  const readSize = useCallback((id: string | null) => {
    const t = threeRef.current;
    if (!t || !id) { setSize([1, 1, 1]); return; }
  ...
  return (
    ...
    >
      ...
      <div className={`flex min-h-0 flex-1 flex-col gap-0 lg:flex-row ${fullscreen ? "" : ""}`}>
        ...
        <div className="relative flex min-h-0 min-w-0 flex-1 flex-col p-2.5 sm:p-3">
          <div
            ref={mountRef}
            className={`relative min-h-0 w-full flex-1 overflow-hidden rounded-xl border border-rose-500/20 bg-[#0c0810] ${fullscreen ? "min-h-[200px]" : "aspect-[5/3.4] min-h-[280px] max-h-[520px] sm:min-h-[340px]"}`}
          >
            {!ready && !error && (
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex flex-wrap items-end justify-between gap-2 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-2.5 pt-8">
              <div className="pointer-events-auto flex flex-wrap gap-1.5">
                  ...
                  <button
                    ...
                    className={
                      mode === m.id
                        ...
                        : "rounded-full border border-white/15 bg-black/50 px-3 py-1.5 text-xs font-semibold text-rose-100/90 backdrop-blur-sm hover:bg-white/15"
                    }
                  >
                    {m.label}
                  </button>
                ...
                <button
                  ...
                  disabled={!selectedId}
                  className="rounded-full border border-rose-500/40 bg-rose-950/70 px-3 py-1.5 text-xs font-semibold text-rose-200 backdrop-blur-sm hover:bg-rose-900/80 disabled:opacity-40"
                >
                  {es ? "Eliminar" : "Delete"}
                </button>
              </div>
              <p className="pointer-events-none hidden text-[10px] text-rose-200/60 sm:block">
                {es ? "Ctrl+C/V · Ctrl+Z · V/R/S · F" : "Ctrl+C/V · Ctrl+Z · V/R/S · F"}
              </p>
            </div>
          </div>
        </div>
        ...
        <aside className="max-h-[42vh] shrink-0 overflow-y-auto border-t border-rose-500/15 bg-black/25 p-2.5 lg:max-h-none lg:w-52 lg:border-l lg:border-t-0">
          ...
          {objects.length === 0 ? (
            ...
            <ul className="space-y-1">
              {objects.map((o) => (
                <li key={o.id}>
                  <button type="button" onClick={() => selectMesh(o.id)}
          ...
          {selectedId && (
            <div className="mt-3 space-y-2 border-t border-rose-500/15 pt-3">
                  ...
                  <div key={axis} className="space-y-1">
                    <div className="flex items-center justify-between gap-1">
                      ...
                      <div className="flex items-center gap-0.5">
                        ...
                        <input type="number" min={0.05} max={50} step={0.05} value={Number(size[axis].toFixed(2))}
                          onChange={(e) => { setSizeAxis(axis, parseFloat(e.target.value) || 0.05); sizeHistPushedRef.current = false; }}
                          className="w-14 rounded border border-white/10 bg-black/40 px-1 py-1 text-center text-xs font-semibold text-rose-50" />
                        <button type="button" className="grid size-6 place-items-center rounded border border-white/10 bg-white/5 text-xs font-bold" onClick={() => { sizeHistPushedRef.current = false; setSizeAxis(axis, size[axis] + 0.1); sizeHistPushedRef.current = false; }}>+</button>
                      </div>
                    </div>
                    <input type="range" min={0.05} max={5} step={0.05} value={Math.min(size[axis], 5)}
                      onChange={(e) => setSizeAxis(axis, parseFloat(e.target.value))}
                      onPointerUp={() => { sizeHistPushedRef.current = false; }}
                      className="h-1.5 w-full cursor-pointer appearance-none rounded-full bg-rose-950 accent-rose-400" />
                  </div>
                );
              })}
              <div className="flex flex-wrap gap-1">
                {[0.5, 1.5, 2].map((f) => (
                  <button key={f} type="button" onClick={() => applySize([size[0] * f, size[1] * f, size[2] * f])}
                    className="rounded-full border border-rose-400/25 bg-rose-950/50 px-2 py-1 text-[10px] font-bold">{f}x</button>
                ))}
