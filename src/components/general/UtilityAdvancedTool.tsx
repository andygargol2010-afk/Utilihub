...
export function UtilityAdvancedTool({ tool, locale = "en" }: { tool: GeneralTool; locale?: "en" | "es" }) {
  ...
  const process = () => {
    ...
    if (tool.slug === "horas-decimales") {
      ...
      if (hours < 0 || minutes < 0 || minutes >= 60 || seconds < 0 || seconds >= 60) { setOut(es ? "Las horas no pueden ser negativas y minutos/segundos deben estar entre 0 y 59." : "Hours cannot be negative and minutes/seconds must be between 0 and 59."); return; }
      const decimal = hours + minutes / 60 + seconds / 3600;
      setOut(es ? `Horas decimales: ${decimal.toFixed(6)}` : `Decimal hours: ${decimal.toFixed(6)}`);
      return;
    }
    setOut(es ? "Herramienta no disponible." : "Tool not available.");
  };

  const labels =
    tool.slug === "propina-y-cuenta-compartida"
      ? es
        ? ["Cuenta", "Propina %", "Personas"]
        : ["Bill", "Tip %", "People"]
      : tool.slug === "regla-50-30-20"
        ? es
          ? ["Ingresos", "Necesidades %", "Deseos %"]
          : ["Income", "Needs %", "Wants %"]
        : es
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-3">{labels.map((label, index) => (
        <label key={label} className="space-y-1"><span className="text-sm font-medium">{label}</span><input type="number" value={values[index] ?? ""} onChange={(event) => update(index, event.target.value)} className={ui?.field ?? "h-11 w-full rounded-xl border bg-background px-3"} /></label>
      ))}</div>
      <button onClick={process} className={ui?.btn ?? "rounded-xl bg-primary px-4 py-2 font-bold text-primary-foreground"}>{es ? "Calcular" : "Calculate"}</button>
      {out && <output className={`block whitespace-pre-wrap rounded-2xl border p-4 ${ui ? `${ui.out} font-medium` : "bg-muted/30"}`}>{out}</output>}
    </div>
  );
}
