...
export function SecurityTool({tool}:{tool:GeneralTool}){
 ...
 const process=async()=>{
  ...
  if(tool.slug==="password-strength"){const bits=entropy(text);const level=bits<40?"Weak":bits<60?"Moderate":bits<80?"Strong":"Very strong";setOut(`${level} · approximate entropy: ${bits.toFixed(1)} bits`);return}
  ...
  if(tool.slug==="comparador-hashes"){if(!expected.trim()){setOut("Enter the expected hash.");return}const actual=await digest("SHA-256",text);setOut(actual.toLowerCase()===expected.trim().toLowerCase()?"Matches SHA-256.":"Does not match SHA-256.");return}
  ...
  setOut("Enter the data and process the tool.");
 };
 return <div className="space-y-4">
  <label className="space-y-1 block"><span className="text-sm font-medium">Text or data</span><textarea value={text} onChange={e=>setText(e.target.value)} className="min-h-32 w-full rounded-xl border bg-background p-4" placeholder="Type or paste here…"/></label>
  {tool.slug==="hmac-sha256"?<label className="space-y-1 block"><span className="text-sm font-medium">Secret key</span><input value={secret} onChange={e=>setSecret(e.target.value)} className="h-11 w-full rounded-xl border bg-background px-3"/></label>:null}
  {tool.slug==="comparador-hashes"?<label className="space-y-1 block"><span className="text-sm font-medium">Expected SHA-256 hash</span><input value={expected} onChange={e=>setExpected(e.target.value)} className="h-11 w-full rounded-xl border bg-background px-3" placeholder="Paste the hash here"/></label>:null}
  <button className="rounded-xl bg-primary px-4 py-2 font-bold text-primary-foreground" onClick={process}>Process</button>
