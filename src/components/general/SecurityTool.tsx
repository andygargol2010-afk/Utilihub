...
export function SecurityTool({tool}:{tool:GeneralTool}){
 const [text,setText]=useState(""),[secret,setSecret]=useState(""),[out,setOut]=useState("");
 const process=async()=>{
  ...
  if(tool.slug==="hmac-sha256"){const key=await crypto.subtle.importKey("raw",new TextEncoder().encode(secret),{name:"HMAC",hash:"SHA-256"},false,["sign"]);const sig=await crypto.subtle.sign("HMAC",key,new TextEncoder().encode(text));setOut([...new Uint8Array(sig)].map(x=>x.toString(16).padStart(2,"0")).join(""));return}
  ...
  if(tool.slug==="comparador-hashes"){setOut("Pega el hash esperado y el texto original en los campos correspondientes para comparar.");return}
