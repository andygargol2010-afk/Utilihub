...
import { ProductivityAdvancedTool } from "./ProductivityAdvancedTool";
...
const UI_RULES: readonly UiRule[] = [
  ...
  { category: "productividad", kind: "productivity-advanced", priority: 3, component: ProductivityAdvancedTool },
...
const duplicateSlugs = GENERAL_TOOLS.reduce<string[]>((duplicates, tool, index, tools) => { if (tools.findIndex((candidate) => candidate.slug === tool.slug) !== index && !duplicates.includes(tool.slug)) duplicates.push(tool.slug); return duplicates; }, []);
if (duplicateSlugs.length > 0) throw new Error(`GENERAL_TOOLS contiene slugs duplicados: ${duplicateSlugs.join(", ")}`);
const matchingRules = (tool: CatalogTool): UiRule[] => UI_RULES.filter((rule) => (rule.category === undefined || rule.category === tool.category) && (rule.kind === undefined || rule.kind === tool.kind));
const resolveToolUi = (tool: CatalogTool): ToolComponent => { const matches = matchingRules(tool); if (matches.length === 0) throw new Error(`GENERAL_TOOL_UI sin implementación explícita para slug "${tool.slug}" (category="${tool.category}", kind="${tool.kind}")`); const specificityScore = (rule: UiRule) => (Number(rule.category !== undefined) + Number(rule.kind !== undefined)) * 10 + (rule.priority ?? 0); const maxScore = Math.max(...matches.map(specificityScore)); const bestMatches = matches.filter((rule) => specificityScore(rule) === maxScore); if (bestMatches.length !== 1) throw new Error(`GENERAL_TOOL_UI ambiguo para slug "${tool.slug}": ${bestMatches.length} reglas con igual prioridad estructural`); return bestMatches[0].component; };
const resolvedComponents = new Map<string, ToolComponent>();
for (const tool of GENERAL_TOOLS) { if (!tool.slug.trim()) throw new Error("GENERAL_TOOLS contiene una herramienta con slug vacío"); if (!tool.name.trim()) throw new Error(`Herramienta "${tool.slug}" sin nombre`); if (!tool.category.trim()) throw new Error(`Herramienta "${tool.slug}" sin categoría`); if (!tool.kind.trim()) throw new Error(`Herramienta "${tool.slug}" sin kind`); resolvedComponents.set(tool.slug, resolveToolUi(tool)); }
export const GENERAL_TOOL_UI: Record<string, () => ReactNode> = Object.fromEntries(GENERAL_TOOLS.map((tool) => { const Component = resolvedComponents.get(tool.slug); if (!Component) throw new Error(`GENERAL_TOOL_UI no pudo resolver "${tool.slug}"`); return [tool.slug, () => <Component tool={tool} />]; })) as Record<string, () => ReactNode>;
