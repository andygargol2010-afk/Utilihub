...
const processText = (slug: string, text: string): string => {
  ...
  switch (slug) {
    ...
    case "contador-de-parrafos": return `Párrafos: ${text.trim() ? text.split(/\r?\n\s*\r?\n/).filter((paragraph) => paragraph.trim()).length : 0}`;
...
export function TextTool({ tool }: { tool: GeneralTool }) {
  ...
  const run = () => {
    try {
      if (tool.slug === "lorem-ipsum") setOut(generateBySlug(tool.slug));
        ...
        setOut(text.replaceAll(secondary, ""));
  ...
  return (
    <div className="space-y-4">
      ...
      {needsSecondary && <label className="block text-sm font-semibold">{tool.slug === "diferencia-textos" ? "Segundo texto" : "Texto a buscar"}<textarea value={secondary} onChange={(event) => setSecondary(event.target.value)} className="mt-2 min-h-32 w-full rounded-xl border border-border bg-background p-4" /></label>}
      ...
      <p className="text-xs text-muted-foreground">{count.words} palabras · {count.chars} caracteres · {count.lines} líneas</p>
