...
type Field={key:string;label:string;type?:"text"|"number"|"file"|"date";placeholder?:string};
...
function fieldsFor(tool:Tool):Field[]{const c=(tool.config||{}) as Partial<Config>;return (c.fields||["text"]).map(key=>({key,label:LABELS[key]||key,type:key==="file"?"file":(["dpi","factor","radius","widthCm","heightCm","bitrate","duration","sizeMB","sampleRate","bits","channels","fps","frames","width","height","scale","lat","lon","lat1","lon1","lat2","lon2","lat3","lon3","distanceKm","bearing","precision","meters","latitude","value"].includes(key)?"number":"text"),placeholder:PLACEHOLDERS[key]||""}));}
...
export function LocalUtilityTool({tool}:{tool:Tool}){
 ...
 return <div className="space-y-4">
  <div className="grid gap-4 sm:grid-cols-2">
   {fields.map(field=><label key={field.key} className="space-y-1">
    <span className="text-sm font-medium">{field.label}</span>
    {field.type==="file"?<input type="file" accept={accept} onChange={e=>setFile(e.target.files?.[0])} className="block h-11 w-full rounded-xl border bg-background px-3 py-2 text-sm"/>:<input type={field.type||"text"} value={values[field.key]||""} onChange={e=>setValues(v=>({...v,[field.key]:e.target.value}))} placeholder={field.placeholder} className="h-11 w-full rounded-xl border bg-background px-3"/>}
   </label>)}
  </div>
  {fields.some(f=>f.key==="text")&&fields.length===1&&<textarea value={values.text||""} onChange={e=>setValues(v=>({...v,text:e.target.value}))} placeholder={PLACEHOLDERS.text} className="min-h-40 w-full rounded-xl border bg-background p-4"/>}
