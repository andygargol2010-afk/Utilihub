...
function calculate(op: string, v: number[][]): string {
  ...
  switch(op){
    ...
    case "chemical-yield":if(B===0)throw Error("El rendimiento teórico no puede ser 0.");return `${A/B*100}%`;
    case "dilution":if(B<=0||C<=0)throw Error("Volumen y concentración deben ser mayores que 0.");return `${A*B/C} L de volumen final`;
    case "mixed-concentration":if(B+D<=0)throw Error("Los volúmenes deben sumar más de 0.");return `${(A*B+C*D)/(B+D)}`;
    ...
    case "water-consumption":return `${A*B} L`;case "recipe-calories":return `${A*B} kcal`;case "cost-per-kg":if(B<=0)throw Error("El peso debe ser mayor que 0.");return `${A/B} por kg`;case "cost-per-liter":if(B<=0)throw Error("El volumen debe ser mayor que 0.");return `${A/B} por L`;case "unit-price":if(B<=0)throw Error("La cantidad debe ser mayor que 0.");return `${A/B} por unidad`;case "split-tip":if(C<=0)throw Error("El número de personas debe ser mayor que 0.");return `Total: ${A*(1+B/100)}; por persona: ${A*(1+B/100)/C}`;case "proportional-share":if(C<=0)throw Error("La suma de partes debe ser mayor que 0.");return `${A*B/C}`;case "box-volume":return `${A*B*C}`;case "container-liters":return `${Math.PI*A*A*B/1000} L`;case "paint-needed":if(B<=0||C<=0)throw Error("Cobertura y capas deben ser mayores que 0.");return `${A*C/B} L`;
