...
const csvToJson = (text: string) => {
  ...
  if (!headers.length || headers.some((h) => !h.trim())) throw new Error("Header cells cannot be empty.");
  ...
  return (
    <div className="space-y-4">
      ...
      <textarea
        ...
        placeholder={
          tool.slug.startsWith("json-") || tool.slug === "csv-a-json"
            ? es
              ? "Pega los datos aquí…"
              : "Paste data here…"
            : es
      ...
      {out && (
        <div className="space-y-2">
          <pre className={`max-h-96 overflow-auto whitespace-pre-wrap break-words rounded-2xl border p-4 text-sm ${ui ? ui.out : "bg-muted/30"}`}>
            {out}
          </pre>
