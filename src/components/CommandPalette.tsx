...
export function CommandPalette() {
  ...
  useEffect(() => {
    if (open) { debouncedSetQuery(""); setSelected(0); requestAnimationFrame(() => inputRef.current?.focus()); }
  }, [open, debouncedSetQuery]);
  useEffect(() => { setSelected(0); }, [query]);

  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[100] grid place-items-start bg-black/40 p-4 pt-[15vh]" role="dialog" aria-modal="true" aria-label="Buscador herramientas" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl border bg-card shadow-2xl">
        <div className="flex items-center gap-3 border-b px-4"><Search className="size-5 text-muted-foreground"/><input ref={inputRef} value={query} onChange={(e) => debouncedSetQuery(e.target.value)} placeholder="Busca una herramienta…" className="h-14 flex-1 bg-transparent outline-none" aria-label="Buscador herramientas"/></div>
        <div className="max-h-[55vh] overflow-y-auto p-2">{results.length ? results.map(({ tool }, index) => <button type="button" key={tool.slug} onMouseEnter={() => setSelected(index)} onClick={() => window.location.assign(toolHref(tool))} className={`flex w-full items-center justify-between rounded-xl px-3 py-3 text-left ${selected === index ? "bg-accent" : ""}`}><span><span className="block font-semibold">{tool.name}</span><span className="text-xs text-muted-foreground">{tool.slug}</span></span><span className="text-xs text-muted-foreground">{tool.category}</span></button>) : <p className="p-6 text-center text-sm text-muted-foreground">No hay herramientas.</p>}</div>
        <div className="border-t px-4 py-2 text-xs text-muted-foreground">↑↓ navegar · ↵ Abrir · Esc cerrar</div>
      </div>
    </div>
  );
}
