...
const navLinkMuted =
  "rounded-full px-3.5 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground";
const activeNav = { className: "bg-accent text-primary ring-1 ring-primary/70" };
const mobileLink =
  "min-h-11 rounded-lg px-3 py-2.5 text-sm font-bold hover:bg-accent";
const mobileLinkMuted =
  "min-h-11 rounded-lg px-3 py-2.5 text-sm font-semibold hover:bg-accent";
    ...
    <header className="site-header sticky top-0 z-50">
      <div className="container-page flex h-16 items-center justify-between gap-3">
        ...
        <nav aria-label="Principal" className="site-nav hidden items-center gap-0.5 lg:flex">
          <Link to="/es" className={navLink} activeOptions={{ exact: true }} activeProps={activeNav}>
            Inicio
          </Link>
          <Link to="/es/herramientas" className={navLink} activeProps={activeNav}>
            Herramientas
          </Link>
          <Link to="/kits" className={navLink} activeProps={activeNav}>
            Kits
          </Link>
          <Link to="/hubs/documentos-y-archivos" className={navLink} activeProps={activeNav}>
            Documentos
          </Link>
          <Link to="/es/finanzas" className={navLink} activeProps={activeNav}>
            Finanzas
          </Link>
      ...
      {open && (
        <nav aria-label="Menú móvil" className="border-t border-border bg-background lg:hidden">
          <div className="container-page grid gap-1 py-2">
            ...
            <Link to="/hubs/documentos-y-archivos" onClick={() => setOpen(false)} className={mobileLink}>
              Documentos y archivos
            </Link>
            <Link to="/es/finanzas" onClick={() => setOpen(false)} className={mobileLink}>
              Finanzas
            </Link>
