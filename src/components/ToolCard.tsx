...
export function ToolCard({tool,isFavorite,onToggleFavorite}:{tool:CatalogTool;isFavorite?:boolean;onToggleFavorite?:(slug:string)=>void}){
 const category=ALL_CATEGORIES.find(c=>c.slug===tool.category)?.name;
 const financial=tool.category==="finanzas";
 return <article className="surface-card group relative flex min-h-44 flex-col p-5 transition-all duration-200 hover:-translate-y-1 hover:border-primary/30 hover:shadow-lift">
  <div className="flex items-start justify-between gap-3"><span className="rounded-full bg-accent px-2.5 py-1 text-[11px] font-bold text-accent-foreground">{category}</span>{onToggleFavorite&&<button type="button" onClick={()=>onToggleFavorite(tool.slug)} aria-pressed={!!isFavorite} aria-label={isFavorite?`Quitar ${tool.name} de favoritos`:`Añadir ${tool.name} de favoritos`} className="relative z-10 rounded-lg border border-transparent p-1.5 text-muted-foreground hover:border-border hover:bg-accent hover:text-highlight"><Star className="size-4" fill={isFavorite?"currentColor":"none"}/></button>}</div>
  <h3 className="mt-5 pr-5 text-lg font-bold">{financial?<Link to="/finanzas/$slug" params={{slug:tool.slug}} className="after:absolute after:inset-0 after:content-[''] group-hover:text-primary">{tool.name}</Link>:<Link to="/herramientas/$slug" params={{slug:tool.slug}} className="after:absolute after:inset-0 after:content-[''] group-hover:text-primary">{tool.name}</Link>}</h3>
