...
export function csvToJson(input: string, separator: CsvSeparator = "comma"): string {
  ...
  if (duplicates.size) {
    throw new Error(`Encabezados duplicados: ${[...duplicates].join(", ")}. Corrige las columnas antes de convertir el CSV.`);
  }

  return JSON.stringify(
    rows.slice(1).map((row, index) => {
      if (row.length !== headers.length) {
        throw new Error(`Fila ${index + 2}: tiene ${row.length} columnas; se esperaban ${headers.length}.`);
      }
...
export function validateCsv(input: string, separator: CsvSeparator = "comma"): string {
  ...
  const errors = rows
    .map((row, index) => row.length !== expected ? `Fila ${index + 1}: ${row.length} columnas (esperadas ${expected}).` : "")
    .filter(Boolean);
...
function tokenizeJsonPath(path: string): string[] {
  ...
  while (i < source.length) {
    ...
    if (content === "*") {
      ...
      throw new Error(`Selector JSON no compatible: [${content}]`);
    }
    i = end + 1;
  }
  return tokens;
...
function compareFilter(item: unknown, expression: string): boolean {
  ...
  try {
    expected = JSON.parse(rawExpected.replace(/^'/, '"').replace(/'$/, '"'));
  } catch {
    ...
    if (token === "*") {
      ...
      for (const value of values) {
        if (Array.isArray(value)) next.push(...value.filter((item) => compareFilter(item, expression)));
          ...
          next.push(...entries.filter((item) => compareFilter(item, expression)));
        }
      }
    } else {
      for (const value of values) {
...
export function regexReplace(pattern: string, text: string, flags: string, replacement: string): string {
  ...
  return text.replace(new RegExp(pattern, validFlags), replacement);
}
...
export function generateHtmlTable(input: string, separator: CsvSeparator): string {
  ...
  if (inconsistent !== -1) {
    throw new Error(`Fila ${inconsistent + 1} inconsistente: tiene ${rows[inconsistent].length} columnas y se esperaban ${expected}. Corrige los datos antes de generar la tabla.`);
  }
