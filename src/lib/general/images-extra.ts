...
export const IMAGE_TOOLS=[
 ...
 i("imagen-pixelada","Pixelar imagen","pixelate","Reduce resolución visual para crear un efecto pixelado.",["file","factor"]),
...
export async function runImages(operation:string,v:Record<string,string>,file?:File):Promise<{text:string;download?:{blob:Blob;filename:string}}>{
 ...
 try{
  ...
  if(operation==="average-color"||operation==="palette"){const scale=Math.min(1,80/Math.max(img.naturalWidth,img.naturalHeight));const w=Math.max(1,Math.round(img.naturalWidth*scale)),h=Math.max(1,Math.round(img.naturalHeight*scale));const [c,ctx]=canvasFor(img,w,h);const data=ctx.getImageData(0,0,c.width,c.height).data;const buckets:Record<string,number>={};let r=0,g=0,b=0,n=0;for(let i=0;i<data.length;i+=4){if(data[i+3]<20)continue;r+=data[i];g+=data[i+1];b+=data[i+2];n++;const key=`${Math.round(data[i]/32)*32},${Math.round(data[i+1]/32)*32},${Math.round(data[i+2]/32)*32}`;buckets[key]=(buckets[key]||0)+1;}if(!n)return{text:"No se encontraron píxeles opacos."};if(operation==="average-color")return{text:`RGB: ${Math.round(r/n)}, ${Math.round(g/n)}, ${Math.round(b/n)}\nHEX: #${[r/n,g/n,b/n].map(x=>Math.round(x).toString(16).padStart(2,"0")).join("")}`};const top=Object.entries(buckets).sort((a,b)=>b[1]-a[1]).slice(0,5).map(([key])=>{const [R,G,B]=key.split(",").map(Number);return`#${[R,G,B].map(x=>Math.min(255,x).toString(16).padStart(2,"0")).join("")}`}).join("\n");return{text:top};}
