...
export async function runFiles(operation:string,values:Record<string,string>,file?:File):Promise<{text:string;download?:{blob:Blob;filename:string}}>{
 ...
 if(operation==="csv-tsv"||operation==="tsv-csv"){const from=operation==="csv-tsv"?",":"\t";const rows=parseDelimited(values.text,from);const out=rows.map(r=>operation==="csv-tsv"?r.join("\t"):r.map(escCsv).join(",")).join("\n");return{text:out};}
 ...
 if(operation==="markdown-toc"){const lines=values.text.split(/\r?\n/);const used=new Map<string,number>();const toc=lines.map(l=>{const m=l.match(/^(#{1,6})\s+(.+)$/);if(!m)return"";const title=m[2].replace(/[*_`]/g,"").trim();const base=title.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9\s-]/g,"").trim().replace(/\s+/g,"-")||"seccion";const count=used.get(base)||0;used.set(base,count+1);const slug=count?`${base}-${count}`:base;return`${"  ".repeat(m[1].length-1)}- [${title}](#${slug})`;}).filter(Boolean).join("\n");return{text:toc||"No se encontraron encabezados."};}
