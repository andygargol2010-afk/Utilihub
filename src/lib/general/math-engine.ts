...
export function calculateMath(slug:string,values:number[],options:{mode?:"aritmetica"|"geometrica";targetBase?:number}={}):MathResult[]{
 ...
 switch(slug){
  ...
  case"regla-de-tres":if(c===0)return[{label:"Error",value:"El valor C debe ser distinto de cero."}];return[{label:"Resultado",value:format(a*b/c)}];
  ...
  case"potencias-y-raices":{if(a===0&&b<0)return[{label:"Error",value:"0 no puede elevarse a un exponente negativo."}];const root=a>=0?Math.sqrt(a):NaN;return[{label:"Potencia",value:format(a**b)},{label:"Raíz cuadrada",value:root===root?format(root):"No definida en números reales"}]}
