...
export function runNetwork(operation:string,v:Record<string,string>):{text:string}{
 ...
 if(operation==="ip-scope"){const p=ipv4(v.text);if(!p)return{text:"IPv4 inválida."};const [a,b]=p;const privateRange=(a===10)||(a===172&&b>=16&&b<=31)||(a===192&&b===168);const special=(a===127)||(a===169&&b===254)||(a>=224);return{text:privateRange?"Privada":special?"Reservada/especial":"Pública"};}
 ...
 if(operation==="ipv6-validate"){try{const u=new URL(`http://[${v.text.trim()}]/`);return{text:u.hostname?"IPv6 válida.":"IPv6 inválida."};}catch{return{text:"IPv6 inválida."};}}
 if(operation==="mac-validate"){const m=v.text.trim().replace(/[-.]/g,":").toUpperCase();const parts=m.split(":");return{text:/^([0-9A-F]{2}:){5}[0-9A-F]{2}$/.test(m)?`MAC válida: ${m}`:"MAC inválida."};}
 ...
 if(operation==="user-agent"){const s=v.text;const browser=/Edg\//.test(s)?"Edge":/Chrome\//.test(s)?"Chrome":/Firefox\//.test(s)?"Firefox":/Safari\//.test(s)&&!/Chrome\//.test(s)?"Safari":/OPR\//.test(s)?"Opera":"No identificado";const os=/Windows NT/.test(s)?"Windows":/Android/.test(s)?"Android":/(iPhone|iPad|CPU OS)/.test(s)?"iOS":/Mac OS X/.test(s)?"macOS":/Linux/.test(s)?"Linux":"No identificado";return{text:`Navegador: ${browser}\nSistema: ${os}`};}
 if(operation==="http-date"){const d=new Date(v.text);if(Number.isNaN(d.getTime())){const n=Number(v.text);if(Number.isFinite(n)){const x=new Date(n*1000);return{text:x.toUTCString()};}return{text:"Fecha o timestamp inválido."};}return{text:d.toISOString()+"\nHTTP: "+d.toUTCString()};}
 if(operation==="http-class"){const code=Number(v.text.trim());return{text:http[code]?`${code} — ${http[code]} (${Math.floor(code/100)}xx)`:code>=100&&code<600?`${Math.floor(code/100)}xx — código HTTP válido no incluido en el catálogo.`:"Código HTTP inválido."};}
 if(operation==="ports"){const p=Number(v.text.trim());return{text:ports[p]?`${p}/tcp o UDP según servicio — ${ports[p]}`:"Puerto no incluido en el catálogo común."};}
 if(operation==="content-type"){const parts=v.text.split(";").map(x=>x.trim());if(!parts[0]||!parts[0].includes("/"))return{text:"Content-Type inválido."};return{text:[`Tipo: ${parts[0]}`,...parts.slice(1).map(x=>`Parámetro: ${x}`)].join("\n")};}
 if(operation==="ipv4-hex"){const p=ipv4(v.text);return{text:p?`0x${u32(p).toString(16).padStart(8,"0")}`:"IPv4 inválida."};}
 if(operation==="ip-sort"){const list=v.text.split(/[\n,;]+/).map(x=>x.trim()).filter(Boolean);const parsed=list.map(x=>[x,ipv4(x)] as const);if(parsed.some(x=>!x[1]))return{text:"La lista contiene una IPv4 inválida."};return{text:parsed.sort((a,b)=>u32(a[1]!)-u32(b[1]!)).map(x=>x[0]).join("\n")};}
