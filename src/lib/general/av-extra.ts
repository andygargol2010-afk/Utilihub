...
export async function runAV(operation:string,v:Record<string,string>,file?:File):Promise<{text:string}>{
 ...
 if(operation==="timecode-frames"){const m=v.timecode.trim().match(/^(\d+):(\d{2}):(\d{2}):(\d{2})$/),fps=Number(v.fps);if(!m||!(Number.isInteger(fps)&&fps>0))return{text:"Usa HH:MM:SS:FF y FPS enteros válidos."};const ff=Number(m[4]);if(ff>=fps)return{text:`El número de frame debe ser menor que ${fps}.`};return{text:String(((Number(m[1])*3600+Number(m[2])*60+Number(m[3]))*fps)+ff)}}
 if(operation==="frames-timecode"){const fr=Number(v.frames),fps=Number(v.fps);if(!(Number.isInteger(fr)&&fr>=0&&Number.isInteger(fps)&&fps>0))return{text:"Frames enteros no negativos y FPS entero positivo requeridos."};const f=fr,ff=f%fps,total=Math.floor(f/fps),s=total%60,m=Math.floor(total/60)%60,h=Math.floor(total/3600);return{text:`${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}:${String(s).padStart(2,"0")}:${String(ff).padStart(2,"0")}`}}
 ...
 if(operation==="seconds-subtitle"){const s=Number(v.text);if(!(s>=0))return{text:"Segundos inválidos."};const rawMs=Math.round((s-Math.floor(s))*1000);const t=Math.floor(s)+(rawMs===1000?1:0),ms=rawMs===1000?0:rawMs,ss=t%60,mm=Math.floor(t/60)%60,hh=Math.floor(t/3600);return{text:`${String(hh).padStart(2,"0")}:${String(mm).padStart(2,"0")}:${String(ss).padStart(2,"0")},${String(ms).padStart(3,"0")}`}}
