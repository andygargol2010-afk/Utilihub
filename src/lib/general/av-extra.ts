...
export async function runAV(operation:string,v:Record<string,string>,file?:File):Promise<{text:string}>{
 ...
 if(operation==="duration-seconds"){const m=v.text.trim().match(/^(\d+):(\d{2}):(\d{2}(?:\.\d+)?)$/);if(!m)return{text:"Usa HH:MM:SS."};const x=Number(m[1])*3600+Number(m[2])*60+Number(m[3]);return{text:Number.isFinite(x)?String(x):"Duración inválida."}}
 ...
 if(operation==="timecode-frames"){const m=v.timecode.trim().match(/^(\d+):(\d{2}):(\d{2}):(\d{2})$/),fps=Number(v.fps);if(!m||!(fps>0))return{text:"Usa HH:MM:SS:FF y FPS válidos."};return{text:String(((Number(m[1])*3600+Number(m[2])*60+Number(m[3]))*fps)+Number(m[4]))}}
 if(operation==="frames-timecode"){const fr=Number(v.frames),fps=Number(v.fps);if(!(fr>=0&&fps>0))return{text:"Frames o FPS inválidos."};const f=Math.floor(fr),ff=f%Math.round(fps),total=Math.floor(f/fps),s=total%60,m=Math.floor(total/60)%60,h=Math.floor(total/3600);return{text:`${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}:${String(s).padStart(2,"0")}:${String(ff).padStart(2,"0")}`}}
 ...
 if(operation==="subtitle-seconds"){const m=v.timecode.trim().match(/^(\d+):(\d{2}):(\d{2})[,.](\d{3})$/);if(!m)return{text:"Usa HH:MM:SS,mmm."};return{text:String(Number(m[1])*3600+Number(m[2])*60+Number(m[3])+Number(m[4])/1000)}}
 if(operation==="seconds-subtitle"){const s=Number(v.text);if(!(s>=0))return{text:"Segundos inválidos."};const ms=Math.round((s-Math.floor(s))*1000);const t=Math.floor(s),ss=t%60,mm=Math.floor(t/60)%60,hh=Math.floor(t/3600);return{text:`${String(hh).padStart(2,"0")}:${String(mm).padStart(2,"0")}:${String(ss).padStart(2,"0")},${String(ms).padStart(3,"0")}`}}
