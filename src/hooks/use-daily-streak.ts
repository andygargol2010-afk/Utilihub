...
function read(): StreakState {
  try {
    ...
    return value && typeof value.count === "number" && typeof value.lastActivity === "number"
      ? value
      : { count: 0, lastActivity: 0 };
  } catch {
...
export function useDailyStreak() {
  ...
  const recordActivity = useCallback(() => {
    const now = Date.now();
    const current = read();
    const diff = current.lastActivity ? dayIndex(now) - dayIndex(current.lastActivity) : -1;

    // Already counted today — avoid rewriting localStorage on every route change
    if (current.lastActivity && diff === 0) {
      setStreak(current.count);
      return;
    }
